...
public class VanillaBlockLoot extends BlockLootSubProvider {
	...
	protected void generate() {
		...
		this.add(
			Blocks.COMPOSTER,
			block -> LootTable.lootTable()
					.withPool(LootPool.lootPool().add(this.applyExplosionDecay(block, LootItem.lootTableItem(Items.COMPOSTER))))
					.withPool(
						LootPool.lootPool()
		...
		this.add(Blocks.COBWEB, block -> createSilkTouchOrShearsDispatchTable(block, this.applyExplosionCondition(block, LootItem.lootTableItem(Items.STRING))));
		this.add(
			Blocks.DEAD_BUSH,
			...
			block -> LootTable.lootTable()
					.withPool(
						LootPool.lootPool()
							.setRolls(ConstantValue.exactly(1.0F))
							.add(
								this.applyExplosionCondition(block, LootItem.lootTableItem(block)).when(LootItemEntityPropertyCondition.entityPresent(LootContext.EntityTarget.THIS))
							)
					)
		);
							...
							.add(
								AlternativesEntry.alternatives(
									AlternativesEntry.alternatives(
											...
											integer -> ((LootPoolSingletonContainer.Builder)LootItem.lootTableItem(Items.SNOWBALL)
														.when(
															LootItemBlockStatePropertyCondition.hasBlockStateProperties(block)
									...
									AlternativesEntry.alternatives(
										SnowLayerBlock.LAYERS.getPossibleValues(),
										integer -> (LootPoolEntryContainer.Builder)(integer == 8
		...
		this.add(
			Blocks.CAMPFIRE,
			block -> createSilkTouchDispatchTable(
					block, this.applyExplosionCondition(block, LootItem.lootTableItem(Items.CHARCOAL).apply(SetItemCountFunction.setCount(ConstantValue.exactly(2.0F))))
				)
		);
		this.add(
		...
		this.add(
			...
			block -> createSilkTouchDispatchTable(
					block, this.applyExplosionCondition(block, LootItem.lootTableItem(Items.SOUL_SOIL).apply(SetItemCountFunction.setCount(ConstantValue.exactly(1.0F))))
				)
		);
		this.add(
			...
			block -> createSilkTouchDispatchTable(
					...
					((LootPoolSingletonContainer.Builder)LootItem.lootTableItem(Items.AMETHYST_SHARD)
						...
						.otherwise(
							this.applyExplosionDecay(block, LootItem.lootTableItem(Items.AMETHYST_SHARD).apply(SetItemCountFunction.setCount(ConstantValue.exactly(2.0F))))
						)
				)
		);
