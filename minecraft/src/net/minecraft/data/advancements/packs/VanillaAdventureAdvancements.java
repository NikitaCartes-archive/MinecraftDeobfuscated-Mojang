...
public class VanillaAdventureAdvancements implements AdvancementSubProvider {
	...
	public void generate(HolderLookup.Provider provider, Consumer<AdvancementHolder> consumer) {
		...
		Advancement.Builder.advancement()
			...
			.addCriterion(
				"kill_pillager", KilledByArrowTrigger.TriggerInstance.crossbowKilled(holderGetter2, EntityPredicate.Builder.entity().of(holderGetter, EntityType.PILLAGER))
			)
		...
		Advancement.Builder.advancement()
			...
			.addCriterion("arbalistic", KilledByArrowTrigger.TriggerInstance.crossbowKilled(holderGetter2, MinMaxBounds.Ints.exactly(5)))
	...
	private static Advancement.Builder smithingWithStyle(Advancement.Builder builder) {
		...
		VanillaRecipeProvider.smithingTrims()
			...
			.forEach(
				trimTemplate -> builder.addCriterion("armor_trimmed_" + trimTemplate.id().location(), RecipeCraftedTrigger.TriggerInstance.craftedItem(trimTemplate.id()))
			);
		return builder;
	}
	...
	private static Advancement.Builder craftingANewLook(Advancement.Builder builder) {
		...
		VanillaRecipeProvider.smithingTrims()
			...
			.forEach(resourceKey -> builder.addCriterion("armor_trimmed_" + resourceKey.location(), RecipeCraftedTrigger.TriggerInstance.craftedItem(resourceKey)));
		return builder;
	}
