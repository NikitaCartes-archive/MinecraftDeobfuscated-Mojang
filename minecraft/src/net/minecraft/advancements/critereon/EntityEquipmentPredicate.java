	...
	public boolean matches(@Nullable Entity entity) {
		if (entity instanceof LivingEntity livingEntity) {
			if (this.head.isPresent() && !((ItemPredicate)this.head.get()).matches(livingEntity.getItemBySlot(EquipmentSlot.HEAD))) {
				...
				return false;
			} else if (this.mainhand.isPresent() && !((ItemPredicate)this.mainhand.get()).matches(livingEntity.getItemBySlot(EquipmentSlot.MAINHAND))) {
				return false;
			} else {
				return !this.offhand.isPresent() || ((ItemPredicate)this.offhand.get()).matches(livingEntity.getItemBySlot(EquipmentSlot.OFFHAND));
			}
		} else {
			return false;
		}
