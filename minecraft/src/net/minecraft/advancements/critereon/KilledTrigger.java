...
public class KilledTrigger extends SimpleCriterionTrigger<KilledTrigger.TriggerInstance> {
	...
	public static record TriggerInstance(
		...
		public boolean matches(ServerPlayer serverPlayer, LootContext lootContext, DamageSource damageSource) {
			if (this.killingBlow.isPresent() && !((DamageSourcePredicate)this.killingBlow.get()).matches(serverPlayer, damageSource)) {
				return false;
			} else {
				return this.entityPredicate.isEmpty() || ((ContextAwarePredicate)this.entityPredicate.get()).matches(lootContext);
			}
		}
