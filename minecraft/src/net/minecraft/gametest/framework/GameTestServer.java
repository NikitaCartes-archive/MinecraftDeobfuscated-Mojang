...
public class GameTestServer extends MinecraftServer {
			...
			new PrimaryLevelData(
				...
				new WorldGenSettings(
					...
					WorldGenSettings.withOverworld(
						registry2,
						DimensionType.defaultDimensions(registry2, registry, registryHolder.registryOrThrow(Registry.NOISE_GENERATOR_SETTINGS_REGISTRY), 0L),
						new FlatLevelSource(FlatLevelGeneratorSettings.getDefault(registry))
					)
				),
				Lifecycle.stable()
			),
	...
	private void startTests(ServerLevel serverLevel) {
		Collection<GameTestInfo> collection = GameTestRunner.runTestBatches(
			this.testBatches, new BlockPos(0, 4, 0), Rotation.NONE, serverLevel, GameTestTicker.SINGLETON, 8
		);
