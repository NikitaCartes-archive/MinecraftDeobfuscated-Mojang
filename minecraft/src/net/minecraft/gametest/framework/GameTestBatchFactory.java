...
public class GameTestBatchFactory {
	...
	public static Collection<GameTestBatch> fromTestFunction(Collection<TestFunction> collection, ServerLevel serverLevel) {
		Map<String, List<TestFunction>> map = (Map)collection.stream().collect(Collectors.groupingBy(TestFunction::batchName));
		return (Collection<GameTestBatch>)map.entrySet()
			.stream()
			.flatMap(
				entry -> {
					String string = (String)entry.getKey();
					Collection<TestFunction> collectionxx = (Collection)entry.getValue();
					MutableInt mutableInt = new MutableInt();
					return Streams.stream(Iterables.partition(collectionxx, 50))
	...
	public static GameTestRunner.GameTestBatcher fromGameTestInfo() {
		return collection -> {
			Map<String, List<GameTestInfo>> map = (Map)collection.stream()
			...
			return (Collection<GameTestBatch>)map.entrySet().stream().flatMap(entry -> {
				String string = (String)entry.getKey();
				Collection<GameTestInfo> collectionxx = (Collection)entry.getValue();
				MutableInt mutableInt = new MutableInt();
				return Streams.stream(Iterables.partition(collectionxx, 50)).map(list -> toGameTestBatch(ImmutableList.copyOf(list), string, mutableInt));
			}).collect(ImmutableList.toImmutableList());
		};
	}
