...
public class Explosion {
	...
	public void explode() {
		...
		int l = Mth.floor(this.x + (double)q + 1.0);
			...
			if (!entity.ignoreExplosion(this)) {
				...
				if (v <= 1.0) {
					...
					if (z != 0.0) {
						...
						double aa = (1.0 - v) * (double)getSeenPercent(vec3, entity);
						double ab;
	...
	public void finalizeExplosion(boolean bl) {
		...
		if (bl2) {
			...
			for(BlockPos blockPos : this.toBlow) {
				this.level.getBlockState(blockPos).onExplosionHit(this.level, blockPos, this, (itemStack, blockPos) -> addOrAppendStack(list, itemStack, blockPos));
			}
