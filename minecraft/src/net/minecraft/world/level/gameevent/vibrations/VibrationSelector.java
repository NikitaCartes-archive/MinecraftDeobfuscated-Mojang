...
public class VibrationSelector {
	public static final Codec<VibrationSelector> CODEC = RecordCodecBuilder.create(
		instance -> instance.group(
					VibrationInfo.CODEC.optionalFieldOf("event").forGetter(vibrationSelector -> vibrationSelector.currentVibrationData.map(Pair::getLeft)),
			...
			if (l != m) {
				...
				if (vibrationInfo.distance() < vibrationInfo2.distance()) {
					...
					return vibrationInfo.distance() > vibrationInfo2.distance()
						? false
						: VibrationListener.getGameEventFrequency(vibrationInfo.gameEvent()) > VibrationListener.getGameEventFrequency(vibrationInfo2.gameEvent());
				}
			}
		}
	}
