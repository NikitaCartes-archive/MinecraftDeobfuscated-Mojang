...
public interface CastleTransformer extends AreaTransformer1, DimensionOffset1Transformer {
	...
	default int applyPixel(BigContext<?> bigContext, Area area, int i, int j) {
		return this.apply(
			bigContext,
			area.get(this.getParentX(i + 1), this.getParentY(j + 0)),
			area.get(this.getParentX(i + 2), this.getParentY(j + 1)),
			area.get(this.getParentX(i + 1), this.getParentY(j + 2)),
			area.get(this.getParentX(i + 0), this.getParentY(j + 1)),
