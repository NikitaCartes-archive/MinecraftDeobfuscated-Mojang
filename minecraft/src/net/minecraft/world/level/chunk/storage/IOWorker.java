...
public class IOWorker implements ChunkScanAccess, AutoCloseable {
	...
	private CompletableFuture<BitSet> getOrCreateOldDataForRegion(int i, int j) {
		...
		synchronized(this.regionCacheForBlender) {
			CompletableFuture<BitSet> completableFuture = (CompletableFuture)this.regionCacheForBlender.getAndMoveToFirst(l);
		...
		return CompletableFuture.supplyAsync(() -> {
			...
			ChunkPos.rangeClosed(chunkPos, chunkPos2).forEach(chunkPosx -> {
				...
				if (tag instanceof CompoundTag compoundTag && this.isOldChunk(compoundTag)) {
					int ixxx = chunkPosx.getRegionLocalZ() * 32 + chunkPosx.getRegionLocalX();
					bitSet.set(ixxx);
				}
			});
			return bitSet;
		...
		CompletableFuture<Void> completableFuture = this.submitTask(
				() -> Either.left(
						CompletableFuture.allOf(
							(CompletableFuture[])this.pendingWrites.values().stream().map(pendingStore -> pendingStore.result).toArray(i -> new CompletableFuture[i])
						)
					)
			)
			.thenCompose(Function.identity());
