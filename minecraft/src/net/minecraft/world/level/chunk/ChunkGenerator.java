...
public abstract class ChunkGenerator {
	...
	public Pair<BlockPos, Holder<Structure>> findNearestMapStructure(
		...
		for(Holder<Structure> holder : holderSet) {
			for(StructurePlacement structurePlacement : chunkGeneratorStructureState.getPlacementsForStructure(holder)) {
				((Set)map.computeIfAbsent(structurePlacement, structurePlacement -> new ObjectArraySet())).add(holder);
			}
		}
	...
	public void applyBiomeDecoration(WorldGenLevel worldGenLevel, ChunkAccess chunkAccess, StructureManager structureManager) {
		...
		if (!SharedConstants.debugVoidTerrain(chunkPos)) {
			...
			Map<Integer, List<Structure>> map = (Map)registry.stream().collect(Collectors.groupingBy(structure -> structure.step().ordinal()));
				...
				for(int k = 0; k < j; ++k) {
					...
					if (k < i) {
						...
						for(Holder<Biome> holder : set) {
							...
							if (k < list3.size()) {
								HolderSet<PlacedFeature> holderSet = (HolderSet)list3.get(k);
