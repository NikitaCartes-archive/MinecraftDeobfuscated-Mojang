...
public class JigsawPlacement {
	...
	private static void addPieces(
		...
		while(placer.placing.hasNext()) {
			JigsawPlacement.PieceState pieceState = (JigsawPlacement.PieceState)placer.placing.next();
									...
									if (bl && boundingBox2.getYSpan() <= 16) {
										n = list2.stream().mapToInt(structureBlockInfox -> {
											if (!boundingBox2.isInside(structureBlockInfox.pos().relative(JigsawBlock.getFrontFacing(structureBlockInfox.state())))) {
												...
												ResourceKey<StructureTemplatePool> resourceKeyxx = readPoolKey(structureBlockInfox, poolAliasLookup);
												Optional<? extends Holder<StructureTemplatePool>> optionalxx = this.pools.getHolder(resourceKeyxx);
												Optional<Holder<StructureTemplatePool>> optional2 = optionalxx.map(holderx -> ((StructureTemplatePool)holderx.value()).getFallback());
												int ixx = optionalxx.map(holderx -> ((StructureTemplatePool)holderx.value()).getMaxSize(this.structureTemplateManager)).orElse(0);
												int jxx = optional2.map(holderx -> ((StructureTemplatePool)holderx.value()).getMaxSize(this.structureTemplateManager)).orElse(0);
