		...
		if (worldGenLevel.enabledFeatures().contains(FeatureFlags.UPDATE_1_20)) {
			...
			for (BlockPos blockPos : objectArrayList) {
				if (i > 0) {
					...
					worldGenLevel.getBlockEntity(blockPos, BlockEntityType.SUSPICIOUS_SAND)
						.ifPresent(
							suspiciousSandBlockEntity -> suspiciousSandBlockEntity.setLootTable(UpdateOneTwentyBuiltInLootTables.DESERT_PYRAMID_ARCHAEOLOGY, blockPos.asLong())
						);
				} else {
