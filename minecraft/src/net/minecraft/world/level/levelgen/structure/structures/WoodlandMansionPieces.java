	...
	static class MansionGrid {
		...
		private void setupThirdFloor() {
			...
			if (list.isEmpty()) {
				...
				int j = simpleGrid.get(tuple.getA(), tuple.getB());
				simpleGrid.set(tuple.getA(), tuple.getB(), j | 4194304);
				Direction direction = this.get1x2RoomDirection(this.baseGrid, tuple.getA(), tuple.getB(), 1, j & 65535);
				...
				if (list2.isEmpty()) {
					this.thirdFloorGrid.set(0, 0, this.thirdFloorGrid.width, this.thirdFloorGrid.height, 5);
					simpleGrid.set(tuple.getA(), tuple.getB(), j);
				} else {
	...
	static class MansionPiecePlacer {
		...
		public void createMansion(
			...
			for(int l = 0; l < 3; ++l) {
				...
				WoodlandMansionPieces.SimpleGrid simpleGrid3 = mansionGrid.floorRooms[l];
				WoodlandMansionPieces.SimpleGrid simpleGrid4 = l == 2 ? simpleGrid2 : simpleGrid;
				...
				for(int o = 0; o < simpleGrid4.height; ++o) {
					for(int p = 0; p < simpleGrid4.width; ++p) {
						boolean bl2 = l == 2 && simpleGrid4.get(p, o) == 3;
						if (simpleGrid4.get(p, o) == 2 || bl2) {
							...
							if (WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid4, p - 1, o) && !mansionGrid.isRoomId(simpleGrid4, p - 1, o, l, s)) {
								list.add(
							...
							if (WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid4, p, o + 1) && !mansionGrid.isRoomId(simpleGrid4, p, o + 1, l, s)) {
							...
							if (r == 65536) {
								...
								Direction direction3 = mansionGrid.get1x2RoomDirection(simpleGrid4, p, o, l, s);
								boolean bl3 = (q & 4194304) == 4194304;
								this.addRoom1x2(list, blockPos4, rotation, direction3, direction2, floorRoomCollections[l], bl3);
		...
		private void createRoof(
			...
			if (simpleGrid2 != null) {
				for(int i = 0; i < simpleGrid.height; ++i) {
					for(int j = 0; j < simpleGrid.width; ++j) {
						...
						var17 = var17.relative(rotation.rotate(Direction.EAST), (j - this.startX) * 8);
						boolean bl = WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid2, j, i);
						if (WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid, j, i) && bl) {
							if (!WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid, j + 1, i)) {
			...
			for(int i = 0; i < simpleGrid.height; ++i) {
				for(int j = 0; j < simpleGrid.width; ++j) {
					...
					var19 = var19.relative(rotation.rotate(Direction.EAST), (j - this.startX) * 8);
					boolean bl = simpleGrid2 != null && WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid2, j, i);
					if (WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid, j, i) && !bl) {
						if (!WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid, j + 1, i)) {
							...
							} else if (WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid, j + 1, i + 1)) {
						...
						if (!WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid, j - 1, i)) {
							BlockPos blockPos3 = var19.relative(rotation.rotate(Direction.EAST), 0);
							...
							if (!WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid, j, i + 1)) {
								...
								BlockPos blockPos4 = blockPos3.relative(rotation.rotate(Direction.SOUTH), 8);
							...
							if (!WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid, j, i - 1)) {
								list.add(
									new WoodlandMansionPieces.WoodlandMansionPiece(this.structureTemplateManager, "roof_corner", blockPos3, rotation.getRotated(Rotation.CLOCKWISE_180))
								);
							} else if (WoodlandMansionPieces.MansionGrid.isHouse(simpleGrid, j - 1, i - 1)) {
								BlockPos blockPos4 = blockPos3.relative(rotation.rotate(Direction.SOUTH), 1);
								list.add(
									new WoodlandMansionPieces.WoodlandMansionPiece(
