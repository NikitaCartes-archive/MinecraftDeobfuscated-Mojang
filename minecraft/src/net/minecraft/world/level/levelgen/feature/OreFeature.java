...
public class OreFeature extends Feature<OreConfiguration> {
	...
	protected boolean doPlace(
		...
		for (int q = 0; q < p; q++) {
			...
			double v = randomSource.nextDouble() * (double)p / 16.0;
	...
	public static boolean canPlaceOre(
		...
		if (!targetBlockState.target.test(blockState, randomSource)) {
			...
			return shouldSkipAirCheck(randomSource, oreConfiguration.discardChanceOnAirExposure) ? true : !isAdjacentToAir(function, mutableBlockPos);
		}
	}
	...
	protected static boolean shouldSkipAirCheck(RandomSource randomSource, float f) {
		if (f <= 0.0F) {
			...
			return f >= 1.0F ? false : randomSource.nextFloat() >= f;
		}
	}
}
