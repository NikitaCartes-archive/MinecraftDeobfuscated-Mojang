...
public class G22 extends SpecialDimensionBase {
	public G22(Level level, DimensionType dimensionType) {
		super(level, dimensionType, 1.0F);
	}

	@Override
	public ChunkGenerator<?> createRandomLevelGenerator() {
		return new G22.Generator(this.level, fixedBiome(Biomes.THE_VOID), NoneGeneratorSettings.INSTANCE);
	}
	...
	public static class Generator extends ChunkGenerator<NoneGeneratorSettings> {
		...
		public void createStructures(BiomeManager biomeManager, ChunkAccess chunkAccess, ChunkGenerator<?> chunkGenerator, StructureManager structureManager) {
			if (this.template == null) {
				this.template = structureManager.getOrCreate(new ResourceLocation("house_of_bob"));
			}
		...
		public void fillFromNoise(LevelAccessor levelAccessor, ChunkAccess chunkAccess) {
			...
			for (int i = 0; i < 16; i++) {
				for (int j = 0; j < 16; j++) {
					for (int k = 0; k < 64; k++) {
						chunkAccess.setBlockState(mutableBlockPos.set(i, k, j), Blocks.DIRT.defaultBlockState(), false);
					}
				}
			}
		}
