...
public class G21 extends SpecialDimensionBase {
	...
	public G21(Level level, DimensionType dimensionType) {
	...
	public static class Generator extends ChunkGenerator<NoneGeneratorSettings> {
		...
		public void createStructures(BiomeManager biomeManager, ChunkAccess chunkAccess, ChunkGenerator<?> chunkGenerator, StructureManager structureManager) {
			if (this.template == null) {
				this.template = structureManager.getOrCreate(new ResourceLocation("command.com"));
			}
		...
		public void fillFromNoise(LevelAccessor levelAccessor, ChunkAccess chunkAccess) {
			...
			for (int i = 0; i < 16; i++) {
				for (int j = 0; j < 16; j++) {
					for (int k = 0; k < 16; k++) {
						chunkAccess.setBlockState(mutableBlockPos.set(i, k, j), Blocks.BLACK_CONCRETE.defaultBlockState(), false);
					}
				}
			}
		}
