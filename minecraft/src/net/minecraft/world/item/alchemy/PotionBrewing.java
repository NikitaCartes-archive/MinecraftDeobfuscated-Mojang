...
public class PotionBrewing {
	...
	protected static boolean hasPotionMix(ItemStack itemStack, ItemStack itemStack2) {
		Optional<Holder<Potion>> optional = ((PotionContents)itemStack.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY)).potion();
		if (optional.isEmpty()) {
			return false;
		} else {
			for(PotionBrewing.Mix<Potion> mix : POTION_MIXES) {
	...
	public static ItemStack mix(ItemStack itemStack, ItemStack itemStack2) {
		if (itemStack2.isEmpty()) {
			...
			Optional<Holder<Potion>> optional = ((PotionContents)itemStack2.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY)).potion();
			if (optional.isEmpty()) {
				return itemStack2;
			} else {
				for(PotionBrewing.Mix<Item> mix : CONTAINER_MIXES) {
				...
				for(PotionBrewing.Mix<Potion> mix : POTION_MIXES) {
					if (mix.from.is((Holder<Potion>)optional.get()) && mix.ingredient.test(itemStack)) {
