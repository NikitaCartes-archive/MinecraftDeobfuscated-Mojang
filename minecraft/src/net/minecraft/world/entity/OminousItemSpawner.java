...
public class OminousItemSpawner extends Entity {
	...
	private void spawnItem() {
		...
		if (!itemStack.isEmpty()) {
			Item direction = itemStack.getItem();
			Entity entity;
			if (direction instanceof ProjectileItem projectileItem) {
				Direction directionx = Direction.DOWN;
				Projectile projectile = projectileItem.asProjectile(level, this.position(), itemStack, directionx);
				ProjectileItem.DispenseConfig dispenseConfig = projectileItem.createDispenseConfig();
				projectileItem.shoot(
					projectile,
					(double)directionx.getStepX(),
					(double)directionx.getStepY(),
					(double)directionx.getStepZ(),
