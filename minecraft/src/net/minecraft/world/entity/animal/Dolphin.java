...
public class Dolphin extends WaterAnimal {
	...
	public void tick() {
		...
		if (!this.isNoAi()) {
			...
			if (this.level.isClientSide && this.isInWater() && this.getDeltaMovement().lengthSqr() > 0.03) {
				...
				for (int i = 0; i < 2; i++) {
					this.level
						.addParticle(ParticleTypes.DOLPHIN, this.x - vec3.x * (double)h + (double)f, this.y - vec3.y, this.z - vec3.z * (double)h + (double)g, 0.0, 0.0, 0.0);
					this.level
						.addParticle(ParticleTypes.DOLPHIN, this.x - vec3.x * (double)h - (double)f, this.y - vec3.y, this.z - vec3.z * (double)h - (double)g, 0.0, 0.0, 0.0);
				}
			}
		...
		for (int i = 0; i < 7; i++) {
			...
			this.level
				.addParticle(
					...
					this.y + 0.2F + (double)(this.random.nextFloat() * this.getBbHeight()),
	...
	static class DolphinSwimToTreasureGoal extends Goal {
		...
		public boolean canContinueToUse() {
			...
			return !new BlockPos((double)blockPos.getX(), this.dolphin.y, (double)blockPos.getZ()).closerThan(this.dolphin.position(), 4.0)
				&& !this.stuck
		...
		public void tick() {
			...
			if (this.dolphin.closeToNextPos() || this.dolphin.getNavigation().isDone()) {
				Vec3 vec3 = RandomPos.getPosTowards(
					this.dolphin, 16, 1, new Vec3((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ()), (float) (Math.PI / 8)
				);
				if (vec3 == null) {
					vec3 = RandomPos.getPosTowards(this.dolphin, 8, 4, new Vec3((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ()));
				}

				if (vec3 != null) {
					BlockPos blockPos2 = new BlockPos(vec3);
					if (!level.getFluidState(blockPos2).is(FluidTags.WATER) || !level.getBlockState(blockPos2).isPathfindable(level, blockPos2, PathComputationType.WATER)) {
						vec3 = RandomPos.getPosTowards(this.dolphin, 8, 5, new Vec3((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ()));
					}
				}
				...
				this.dolphin.getLookControl().setLookAt(vec3.x, vec3.y, vec3.z, (float)(this.dolphin.getMaxHeadYRot() + 20), (float)this.dolphin.getMaxHeadXRot());
	...
	class PlayWithItemsGoal extends Goal {
		...
		private void drop(ItemStack itemStack) {
			if (!itemStack.isEmpty()) {
				double d = Dolphin.this.y - 0.3F + (double)Dolphin.this.getEyeHeight();
