...
public class Dolphin extends WaterAnimal {
	...
	public void tick() {
		...
		if (this.isNoAi()) {
			...
			if (this.getVehicle() instanceof Player player && player.isSwimming()) {
	...
	static class DolphinSwimToTreasureGoal extends Goal {
		...
		public boolean canContinueToUse() {
			...
			return !new BlockPos((double)blockPos.getX(), this.dolphin.getY(), (double)blockPos.getZ()).closerToCenterThan(this.dolphin.position(), 4.0)
				&& !this.stuck
