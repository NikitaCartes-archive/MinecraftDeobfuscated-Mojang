...
public class EnderDragon extends Mob implements Enemy {
		...
		if (this.isDeadOrDying()) {
			...
			if (this.isNoAi()) {
				...
				if (this.level.isClientSide) {
					...
					Optional<Entity> optional = this.getPassengers().stream().findFirst();
					if (optional.isPresent() && optional.get() instanceof Player player) {
						vec32 = this.position().add(player.getViewVector(1.0F).scale(30.0));
					}

					if (vec32 != null) {
						double l = vec32.x - this.getX();
						double m = vec32.y - this.getY();
						double n = vec32.z - this.getZ();
						double o = l * l + m * m + n * n;
						float p = dragonPhaseInstance.getFlySpeed();
						double q = Math.sqrt(l * l + n * n);
						if (q > 0.0) {
							m = Mth.clamp(m / q, (double)(-p), (double)p);
						}

						this.setDeltaMovement(this.getDeltaMovement().add(0.0, m * 0.01, 0.0));
						this.setYRot(Mth.wrapDegrees(this.getYRot()));
						...
						Vec3 vec34 = new Vec3(
							...
							.normalize();
						float r = Math.max(((float)vec34.dot(vec33) + 0.5F) / 1.5F, 0.0F);
						if (Math.abs(l) > 1.0E-5F || Math.abs(n) > 1.0E-5F) {
							float s = Mth.clamp(Mth.wrapDegrees(180.0F - (float)Mth.atan2(l, n) * (180.0F / (float)Math.PI) - this.getYRot()), -50.0F, 50.0F);
							this.yRotA *= 0.8F;
							this.yRotA = this.yRotA + s * dragonPhaseInstance.getTurnSpeed();
							this.setYRot(this.getYRot() + this.yRotA * 0.1F);
						}

						float s = (float)(2.0 / (o + 1.0));
						float t = 0.06F;
						this.moveRelative(0.06F * (r * s + (1.0F - s)), new Vec3(0.0, 0.0, -1.0));
						if (this.inWall) {
						...
						double u = 0.8 + 0.15 * (vec35.dot(vec34) + 1.0) / 2.0;
						this.setDeltaMovement(this.getDeltaMovement().multiply(u, 0.91F, u));
					}
				}
				...
				for (int v = 0; v < this.subEntities.length; v++) {
					vec3s[v] = new Vec3(this.subEntities[v].getX(), this.subEntities[v].getY(), this.subEntities[v].getZ());
				}

				float w = (float)(this.getLatencyPos(5, 1.0F)[1] - this.getLatencyPos(10, 1.0F)[1]) * 10.0F * (float) (Math.PI / 180.0);
				float x = Mth.cos(w);
				float y = Mth.sin(w);
				float z = this.getYRot() * (float) (Math.PI / 180.0);
				float aa = Mth.sin(z);
				float ab = Mth.cos(z);
				this.tickPart(this.body, (double)(aa * 0.5F), 0.0, (double)(-ab * 0.5F));
				this.tickPart(this.wing1, (double)(ab * 4.5F), 2.0, (double)(aa * 4.5F));
				this.tickPart(this.wing2, (double)(ab * -4.5F), 2.0, (double)(aa * -4.5F));
				...
				float ac = Mth.sin(this.getYRot() * (float) (Math.PI / 180.0) - this.yRotA * 0.01F);
				float ad = Mth.cos(this.getYRot() * (float) (Math.PI / 180.0) - this.yRotA * 0.01F);
				float ae = this.getHeadYOffset();
				this.tickPart(this.head, (double)(ac * 6.5F * x), (double)(ae + y * 6.5F), (double)(-ad * 6.5F * x));
				this.tickPart(this.neck, (double)(ac * 5.5F * x), (double)(ae + y * 5.5F), (double)(-ad * 5.5F * x));
				...
				for (int af = 0; af < 3; af++) {
					...
					double[] es = this.getLatencyPos(12 + af * 2, 1.0F);
					float ag = this.getYRot() * (float) (Math.PI / 180.0) + this.rotWrap(es[0] - ds[0]) * (float) (Math.PI / 180.0);
					float ah = Mth.sin(ag);
					float rx = Mth.cos(ag);
					float s = 1.5F;
					float t = (float)(af + 1) * 2.0F;
					this.tickPart(enderDragonPart, (double)(-(aa * 1.5F + ah * t) * x), es[1] - ds[1] - (double)((t + 1.5F) * y) + 1.5, (double)((ab * 1.5F + rx * t) * x));
				}
				...
				for (int af = 0; af < this.subEntities.length; af++) {
					this.subEntities[af].xo = vec3s[af].x;
					this.subEntities[af].yo = vec3s[af].y;
					this.subEntities[af].zo = vec3s[af].z;
					this.subEntities[af].xOld = vec3s[af].x;
					this.subEntities[af].yOld = vec3s[af].y;
					this.subEntities[af].zOld = vec3s[af].z;
				}
			}
		}
	}
	...
	public void positionRider(Entity entity) {
		...
		entity.setPos(this.getX(), this.getY(0.5) - 1.5, this.getZ());
	}
