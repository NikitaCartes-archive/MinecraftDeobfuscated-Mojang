...
public class LocateHidingPlace extends Behavior<LivingEntity> {
	...
	protected boolean checkExtraStartConditions(ServerLevel serverLevel, LivingEntity livingEntity) {
		Optional<BlockPos> optional = serverLevel.getPoiManager()
			.find(holder -> holder.is(PoiTypes.HOME), blockPos -> true, livingEntity.blockPosition(), this.closeEnoughDist + 1, PoiManager.Occupancy.ANY);
	...
	protected void start(ServerLevel serverLevel, LivingEntity livingEntity, long l) {
		...
		if (optional.isEmpty()) {
			optional = serverLevel.getPoiManager()
				.getRandom(
					holder -> holder.is(PoiTypes.HOME), blockPos -> true, PoiManager.Occupancy.ANY, livingEntity.blockPosition(), this.radius, livingEntity.getRandom()
				);
			if (optional.isEmpty()) {
