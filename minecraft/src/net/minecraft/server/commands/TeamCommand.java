...
public class TeamCommand {
					...
					Commands.literal("add")
						.then(
							((RequiredArgumentBuilder)Commands.argument("team", StringArgumentType.word())
									.executes(commandContext -> createTeam((CommandSourceStack)commandContext.getSource(), StringArgumentType.getString(commandContext, "team"))))
								.then(
							...
							((RequiredArgumentBuilder)Commands.argument("team", TeamArgument.team())
									.executes(
										commandContext -> joinTeam(
												...
												Collections.singleton(((CommandSourceStack)commandContext.getSource()).getEntityOrException())
											)
									))
								.then(
													...
													Commands.literal("nametagVisibility")
														.then(
															Commands.literal("never")
																.executes(
																	commandContext -> setNametagVisibility(commandContext.getSource(), TeamArgument.getTeam(commandContext, "team"), Team.Visibility.NEVER)
																)
														)
														.then(
														...
														.then(
															Commands.literal("always")
																.executes(
																	commandContext -> setNametagVisibility(commandContext.getSource(), TeamArgument.getTeam(commandContext, "team"), Team.Visibility.ALWAYS)
																)
														)
												))
											.then(
	...
	private static int joinTeam(CommandSourceStack commandSourceStack, PlayerTeam playerTeam, Collection<ScoreHolder> collection) {
		...
		if (collection.size() == 1) {
			...
			commandSourceStack.sendSuccess(
				() -> Component.translatable("commands.team.join.success.multiple", collection.size(), playerTeam.getFormattedDisplayName()), true
			);
		}
	...
	private static int setNametagVisibility(CommandSourceStack commandSourceStack, PlayerTeam playerTeam, Team.Visibility visibility) throws CommandSyntaxException {
		if (playerTeam.getNameTagVisibility() == visibility) {
			...
			commandSourceStack.sendSuccess(
				() -> Component.translatable("commands.team.option.nametagVisibility.success", playerTeam.getFormattedDisplayName(), visibility.getDisplayName()), true
			);
			return 0;
		}
	}
	...
	private static int setDeathMessageVisibility(CommandSourceStack commandSourceStack, PlayerTeam playerTeam, Team.Visibility visibility) throws CommandSyntaxException {
		if (playerTeam.getDeathMessageVisibility() == visibility) {
			...
			commandSourceStack.sendSuccess(
				() -> Component.translatable("commands.team.option.deathMessageVisibility.success", playerTeam.getFormattedDisplayName(), visibility.getDisplayName()),
				true
			);
			return 0;
		}
	...
	private static int setCollision(CommandSourceStack commandSourceStack, PlayerTeam playerTeam, Team.CollisionRule collisionRule) throws CommandSyntaxException {
		if (playerTeam.getCollisionRule() == collisionRule) {
			...
			commandSourceStack.sendSuccess(
				() -> Component.translatable("commands.team.option.collisionRule.success", playerTeam.getFormattedDisplayName(), collisionRule.getDisplayName()), true
			);
			return 0;
		}
	}
	...
	private static int setFriendlySight(CommandSourceStack commandSourceStack, PlayerTeam playerTeam, boolean bl) throws CommandSyntaxException {
		if (playerTeam.canSeeFriendlyInvisibles() == bl) {
			...
			commandSourceStack.sendSuccess(
				() -> Component.translatable("commands.team.option.seeFriendlyInvisibles." + (bl ? "enabled" : "disabled"), playerTeam.getFormattedDisplayName()), true
			);
			return 0;
		}
	}
	...
	private static int setFriendlyFire(CommandSourceStack commandSourceStack, PlayerTeam playerTeam, boolean bl) throws CommandSyntaxException {
		if (playerTeam.isAllowFriendlyFire() == bl) {
			...
			commandSourceStack.sendSuccess(
				() -> Component.translatable("commands.team.option.friendlyfire." + (bl ? "enabled" : "disabled"), playerTeam.getFormattedDisplayName()), true
			);
			return 0;
		}
	}
	...
	private static int setColor(CommandSourceStack commandSourceStack, PlayerTeam playerTeam, ChatFormatting chatFormatting) throws CommandSyntaxException {
		if (playerTeam.getColor() == chatFormatting) {
			...
			commandSourceStack.sendSuccess(
				() -> Component.translatable("commands.team.option.color.success", playerTeam.getFormattedDisplayName(), chatFormatting.getName()), true
			);
			return 0;
		}
	}
	...
	private static int createTeam(CommandSourceStack commandSourceStack, String string) throws CommandSyntaxException {
		return createTeam(commandSourceStack, string, Component.literal(string));
	}
