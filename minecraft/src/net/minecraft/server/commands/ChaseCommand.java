			...
			Commands.literal("chase")
				...
				.then(
					Commands.literal("lead")
						...
						.executes(commandContext -> lead(commandContext.getSource(), "localhost", 10000))
				)
		...
		if (alreadyRunning(commandSourceStack)) {
			...
			try {
				...
				commandSourceStack.sendSuccess(
					new TextComponent("Chase server is now running on port " + i + ". Clients can follow you using /chase follow <ip> <port>"), false
				);
