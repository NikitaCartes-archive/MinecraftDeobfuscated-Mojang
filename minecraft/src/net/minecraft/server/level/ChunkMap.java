...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider {
		...
		if (i == 0) {
			...
			CompletableFuture<ChunkResult<List<ChunkAccess>>> completableFuture3 = completableFuture2.thenApply(listx -> {
				...
				for(ChunkResult<ChunkAccess> chunkResult : listx) {
					...
					list2xx.add(chunkAccess);
					++l;
				}

				return ChunkResult.of(list2xx);
			});
	...
	private CompletableFuture<ChunkResult<ChunkAccess>> scheduleChunkGeneration(ChunkHolder chunkHolder, ChunkStatus chunkStatus) {
		...
		return completableFuture.thenComposeAsync(chunkResult -> {
			List<ChunkAccess> list = (List)chunkResult.orElse(null);
			if (list == null) {
				...
				try {
					...
					CompletableFuture<ChunkAccess> completableFuturexx;
					if (chunkAccess.getStatus().isOrAfter(chunkStatus)) {
						completableFuturexx = chunkStatus.load(this.worldGenContext, chunkAccess -> this.protoChunkToFullChunk(chunkHolder, chunkAccess), chunkAccess);
					} else {
						completableFuturexx = chunkStatus.generate(this.worldGenContext, executor, chunkAccess -> this.protoChunkToFullChunk(chunkHolder, chunkAccess), list);
					}

					this.progressListener.onStatusChange(chunkPos, chunkStatus);
					return completableFuturexx.thenApply(ChunkResult::of);
	...
	private static String printFuture(CompletableFuture<ChunkResult<LevelChunk>> completableFuture) {
		try {
			ChunkResult<LevelChunk> chunkResult = (ChunkResult)completableFuture.getNow(null);
			if (chunkResult != null) {
