...
public record Filterable<T>(T raw, Optional<T> filtered) {
	...
	public static Filterable<String> from(FilteredText filteredText) {
		return new Filterable((T)filteredText.raw(), filteredText.isFiltered() ? Optional.of(filteredText.filteredOrEmpty()) : Optional.empty());
	}
	...
	public <U> Optional<Filterable<U>> resolve(Function<T, Optional<U>> function) {
		...
		if (optional.isEmpty()) {
			...
			Optional<U> optional2 = (Optional)function.apply(this.filtered.get());
			return optional2.isEmpty() ? Optional.empty() : Optional.of(new Filterable<Object>(optional.get(), optional2));
		} else {
			return Optional.of(new Filterable<Object>(optional.get(), Optional.empty()));
		}
	}
}
