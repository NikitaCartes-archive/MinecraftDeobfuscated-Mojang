...
public class RegistrySetBuilder {
	...
	public <T> RegistrySetBuilder add(
		...
		this.entries.add(new RegistrySetBuilder.RegistryStub<T>(resourceKey, lifecycle, registryBootstrap));
		return this;
	}
	...
	private HolderLookup.Provider createLazyFullPatchedRegistries(
		...
		List<HolderLookup.RegistryLookup<?>> list = (List)map.keySet()
			.stream()
	...
	private <T> HolderLookup.RegistryLookup<T> createLazyFullPatchedRegistries(
		...
		if (cloner == null) {
			...
			registryLookup.listElements().forEach(reference -> {
				ResourceKey<T> resourceKeyxx = reference.key();
				RegistrySetBuilder.LazyHolder<T> lazyHolder = new RegistrySetBuilder.LazyHolder<>(holderOwner, resourceKeyxx);
				lazyHolder.supplier = () -> cloner.clone((T)reference.value(), provider, mutableObject.getValue());
				map.put(resourceKeyxx, lazyHolder);
			});
			...
			registryLookup2.listElements().forEach(reference -> {
				ResourceKey<T> resourceKeyxx = reference.key();
				map.computeIfAbsent(resourceKeyxx, resourceKey2 -> {
	...
	public RegistrySetBuilder.PatchedRegistries buildPatch(RegistryAccess registryAccess, HolderLookup.Provider provider, Cloner.Factory factory) {
		...
		Set<ResourceKey<? extends Registry<?>>> set = (Set)registryAccess.listRegistries().collect(Collectors.toUnmodifiableSet());
	...
	static record BuildState(
				...
				public Holder.Reference<T> register(ResourceKey<T> resourceKey, T object, Lifecycle lifecycle) {
					...
					if (registeredValue != null) {
						BuildState.this.errors
							.add(new IllegalStateException("Duplicate registration for " + resourceKey + ", new=" + object + ", old=" + registeredValue.value));
					}
		...
		public void reportUnclaimedRegisteredValues() {
			this.registeredValues
				.forEach(
					(resourceKey, registeredValue) -> this.errors.add(new IllegalStateException("Orpaned value " + registeredValue.value + " for key " + resourceKey))
				);
		}
	...
	static class LazyHolder<T> extends Holder.Reference<T> {
		...
		protected LazyHolder(HolderOwner<T> holderOwner, @Nullable ResourceKey<T> resourceKey) {
			super(Holder.Reference.Type.STAND_ALONE, holderOwner, resourceKey, (T)null);
		}
	...
	static record RegistryContents<T>(
		...
		public HolderLookup.RegistryLookup<T> buildAsLookup(RegistrySetBuilder.CompositeOwner compositeOwner) {
			Map<ResourceKey<T>, Holder.Reference<T>> map = (Map)this.values
				.entrySet()
				.stream()
				.collect(
					Collectors.toUnmodifiableMap(
						Entry::getKey,
						entry -> {
							RegistrySetBuilder.ValueAndHolder<T> valueAndHolder = (RegistrySetBuilder.ValueAndHolder)entry.getValue();
							Holder.Reference<T> reference = (Holder.Reference)valueAndHolder.holder()
	...
	static record RegistryStub<T>(ResourceKey<? extends Registry<T>> key, Lifecycle lifecycle, RegistrySetBuilder.RegistryBootstrap<T> bootstrap) {
		...
		public RegistrySetBuilder.RegistryContents<T> collectRegisteredValues(RegistrySetBuilder.BuildState buildState) {
			...
			while(iterator.hasNext()) {
				Entry<ResourceKey<?>, RegistrySetBuilder.RegisteredValue<?>> entry = (Entry)iterator.next();
