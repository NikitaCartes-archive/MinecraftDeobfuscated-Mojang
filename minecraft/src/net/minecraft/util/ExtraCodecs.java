				...
				return dynamicOps.getMap(object)
					.flatMap(
						mapLike -> {
							...
							DataResult<com.mojang.datafixers.util.Unit> dataResult = (DataResult)mapLike.entries()
								.reduce(
									...
									(dataResultx, pair) -> {
										...
										if (optional.isPresent()) {
											V objectxxx = (V)map.putIfAbsent(((Pair)optional.get()).getFirst(), ((Pair)optional.get()).getSecond());
											if (objectxxx != null) {
												builder.add(pair);
							...
							return dataResult.<Pair>map(unit -> Pair.of(map2, object)).setPartial(Pair.of(map2, object)).mapError(string -> string + " missed input: " + object2);
						}
					);
			}
		};
