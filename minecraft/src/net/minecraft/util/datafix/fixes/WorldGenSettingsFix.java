...
public class WorldGenSettingsFix extends DataFix {
	...
	private static <T> Dynamic<T> fix(Dynamic<T> dynamic) {
		...
		Optional<String> optional = dynamic.get("generatorName").asString().map(string -> string.toLowerCase(Locale.ROOT)).result();
		Optional<String> optional2 = (Optional)dynamic.get("legacy_custom_options")
			.asString()
			.result()
			.map(Optional::of)
		...
		if (optional.equals(Optional.of("customized"))) {
			...
			switch(bl6) {
				...
				case "buffet":
					...
					Dynamic<T> dynamic4 = (Dynamic)optionalDynamic2.get("biome_source")
						.result()
