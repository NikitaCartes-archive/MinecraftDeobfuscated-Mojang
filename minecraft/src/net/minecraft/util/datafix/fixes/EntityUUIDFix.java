...
public class EntityUUIDFix extends AbstractUUIDFix {
	...
	protected TypeRewriteRule makeRule() {
		return this.fixTypeEverywhereTyped("EntityUUIDFixes", this.getInputSchema().getType(this.typeReference), typed -> {
			...
			for (String string : ABSTRACT_HORSES) {
				typed = this.updateNamedChoice(typed, string, this::updateAnimalOwner);
			}
			...
			for (String string : ANIMALS) {
				typed = this.updateNamedChoice(typed, string, this::updateAnimal);
			}

			for (String string : MOBS) {
				typed = this.updateNamedChoice(typed, string, this::updateMob);
			}

			for (String string : LIVING_ENTITIES) {
				typed = this.updateNamedChoice(typed, string, this::updateLivingEntity);
			}

			for (String string : PROJECTILES) {
				typed = this.updateNamedChoice(typed, string, this::updateProjectile);
			}

			typed = this.updateNamedChoice(typed, "minecraft:bee", this::updateHurtBy);
			typed = this.updateNamedChoice(typed, "minecraft:zombified_piglin", this::updateHurtBy);
			typed = this.updateNamedChoice(typed, "minecraft:fox", this::updateFox);
			typed = this.updateNamedChoice(typed, "minecraft:item", this::updateItem);
			typed = this.updateNamedChoice(typed, "minecraft:shulker_bullet", this::updateShulkerBullet);
			typed = this.updateNamedChoice(typed, "minecraft:area_effect_cloud", this::updateAreaEffectCloud);
			typed = this.updateNamedChoice(typed, "minecraft:zombie_villager", this::updateZombieVillager);
			typed = this.updateNamedChoice(typed, "minecraft:evoker_fangs", this::updateEvokerFangs);
			return this.updateNamedChoice(typed, "minecraft:piglin", this::updatePiglin);
		});
	}

	private Dynamic<?> updatePiglin(Dynamic<?> dynamic) {
	...
	private Dynamic<?> updateMob(Dynamic<?> dynamic) {
		return this.updateLivingEntity(dynamic).update("Leash", dynamicx -> (Dynamic)replaceUUIDLeastMost(dynamicx, "UUID", "UUID").orElse(dynamicx));
	}
