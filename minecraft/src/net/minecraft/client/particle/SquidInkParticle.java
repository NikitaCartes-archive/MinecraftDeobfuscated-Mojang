...
public class SquidInkParticle extends SimpleAnimatedParticle {
	SquidInkParticle(ClientLevel clientLevel, double d, double e, double f, double g, double h, double i, int j, SpriteSet spriteSet) {
		...
		this.quadSize = 0.5F;
		this.setAlpha(1.0F);
		this.setColor((float)ARGB.red(j), (float)ARGB.green(j), (float)ARGB.blue(j));
	...
	public static class Provider implements ParticleProvider<SimpleParticleType> {
		...
		public Particle createParticle(SimpleParticleType simpleParticleType, ClientLevel clientLevel, double d, double e, double f, double g, double h, double i) {
			return new SquidInkParticle(clientLevel, d, e, f, g, h, i, ARGB.color(255, 255, 255, 255), this.sprites);
		}
	}
}
