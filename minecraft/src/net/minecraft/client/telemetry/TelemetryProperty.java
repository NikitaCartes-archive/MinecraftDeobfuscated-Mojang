...
public record TelemetryProperty<T>(String id, String exportKey, Codec<T> codec, TelemetryProperty.Exporter<T> exporter) {
	...
	public static TelemetryProperty<GameLoadTimesEvent.Measurement> gameLoadMeasurement(String string, String string2) {
		return create(
			string,
			string2,
			GameLoadTimesEvent.Measurement.CODEC,
			(telemetryPropertyContainer, stringx, measurement) -> telemetryPropertyContainer.addProperty(stringx, measurement.millis())
		);
	}
	...
	public static enum GameMode implements StringRepresentable {
		...
		private GameMode(final String string2, final int j) {
			this.key = string2;
			this.id = j;
		}
	...
	public static enum ServerType implements StringRepresentable {
		...
		private ServerType(final String string2) {
			this.key = string2;
		}
