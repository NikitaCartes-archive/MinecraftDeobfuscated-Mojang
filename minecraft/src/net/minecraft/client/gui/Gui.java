	...
	private void displayScoreboardSidebar(GuiGraphics guiGraphics, Objective objective) {
		...
		DisplayEntry[] lvs = (DisplayEntry[])scoreboard.listPlayerScores(objective)
			...
			.map(playerScoreEntry -> {
				...
				Component componentxx = playerScoreEntry.ownerName();
				Component component2 = PlayerTeam.formatNameForTeam(playerTeam, componentxx);
				Component component3 = playerScoreEntry.formatValue(numberFormat);
				int ixx = this.getFont().width(component3);
				return new DisplayEntry(component2, component3, ixx);
			})
			.toArray(ix -> new DisplayEntry[ix]);
		...
		guiGraphics.drawManaged(() -> {
			...
			for(int t = 0; t < kxx; ++t) {
				DisplayEntry lvxx = lvs[t];
				int u = m - (kxx - t) * 9;
				guiGraphics.drawString(this.getFont(), lvxx.name, o, u, -1, false);
				guiGraphics.drawString(this.getFont(), lvxx.score, p - lvxx.scoreWidth, u, -1, false);
			}
		});
	}
