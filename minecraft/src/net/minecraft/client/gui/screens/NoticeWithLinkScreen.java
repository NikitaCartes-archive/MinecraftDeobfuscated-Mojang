...
public class NoticeWithLinkScreen extends Screen {
	...
	protected void init() {
		...
		rowHelper2.addChild(Button.builder(CommonComponents.GUI_OPEN_IN_BROWSER, button -> Util.getPlatform().openUri(this.url)).size(120, 20).build());
		rowHelper2.addChild(
			Button.builder(CommonComponents.GUI_COPY_LINK_TO_CLIPBOARD, button -> this.minecraft.keyboardHandler.setClipboard(this.url)).size(120, 20).build()
		);
