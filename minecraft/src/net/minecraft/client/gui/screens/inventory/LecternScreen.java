...
public class LecternScreen extends BookViewScreen implements MenuAccess<LecternMenu> {
	...
	protected void createMenuControls() {
		if (this.minecraft.player.mayBuild()) {
			this.addButton(new Button(this.width / 2 - 100, 196, 98, 20, I18n.get("gui.done"), button -> this.minecraft.setScreen(null)));
			this.addButton(new Button(this.width / 2 + 2, 196, 98, 20, I18n.get("lectern.take_book"), button -> this.sendButtonClick(3)));
		} else {
	...
	private void bookChanged() {
		...
		this.setBookAccess(BookAccess.fromItem(itemStack));
	}
