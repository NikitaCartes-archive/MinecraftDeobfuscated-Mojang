...
public class UnihexProvider implements GlyphProvider {
	...
	public static class Definition implements GlyphProviderDefinition {
		...
		private UnihexProvider loadData(InputStream inputStream) throws IOException {
			CodepointMap<UnihexProvider.LineData> codepointMap = new CodepointMap<>(ix -> new UnihexProvider.LineData[ix], ix -> new UnihexProvider.LineData[ix][]);
			...
			try {
				...
				CodepointMap<UnihexProvider.Glyph> codepointMap2 = new CodepointMap<>(ix -> new UnihexProvider.Glyph[ix], ix -> new UnihexProvider.Glyph[ix][]);
				...
				codepointMap.forEach((ix, lineDatax) -> {
					int jxx = lineDatax.calculateWidth();
					int kxx = UnihexProvider.Dimensions.left(jxx);
					int l = UnihexProvider.Dimensions.right(jxx);
					codepointMap2.put(ix, new UnihexProvider.Glyph(lineDatax, kxx, l));
				});
