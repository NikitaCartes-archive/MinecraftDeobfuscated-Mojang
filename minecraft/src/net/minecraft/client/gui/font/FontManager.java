...
public class FontManager implements PreparableReloadListener, AutoCloseable {
	...
	private CompletableFuture<FontManager.Preparation> prepare(ResourceManager resourceManager, Executor executor) {
		...
		for(Entry<ResourceLocation, List<Resource>> entry : FONT_DEFINITIONS.listMatchingResourceStacks(resourceManager).entrySet()) {
			...
			list.add(CompletableFuture.supplyAsync(() -> {
				List<Pair<FontManager.BuilderId, GlyphProviderDefinition.Conditional>> listxx = loadResourceStack((List<Resource>)entry.getValue(), resourceLocation);
				FontManager.UnresolvedBuilderBundle unresolvedBuilderBundle = new FontManager.UnresolvedBuilderBundle(resourceLocation);

				for(Pair<FontManager.BuilderId, GlyphProviderDefinition.Conditional> pair : listxx) {
		...
		return Util.sequence(list)
			.thenCompose(
				listx -> {
					List<CompletableFuture<Optional<GlyphProvider>>> list2 = (List)listx.stream()
						...
						.thenCompose(
							list2x -> {
								...
								return CompletableFuture.allOf(completableFutures).thenApply(void_ -> {
									List<GlyphProvider> list2xxx = list2x.stream().flatMap(Optional::stream).toList();
									return new FontManager.Preparation(map, list2xxx);
								});
							}
						);
				}
	...
	private CompletableFuture<Optional<GlyphProvider>> safeLoad(
		...
		return CompletableFuture.supplyAsync(() -> {
			try {
				...
				LOGGER.warn("Failed to load builder {}, rejecting", builderId, var4xx);
	...
	private void finalizeProviderLoading(List<GlyphProvider.Conditional> list, GlyphProvider.Conditional conditional) {
		...
		intSet.forEach(i -> {
			if (i != 32) {
				for(GlyphProvider.Conditional conditionalxx : Lists.reverse(list)) {
					if (conditionalxx.provider().getGlyph(i) != null) {
						break;
					}
				}
			}
		...
		for(Resource resource : list) {
			try {
				...
				try {
					...
					FontManager.FontDefinitionFile fontDefinitionFile = (FontManager.FontDefinitionFile)FontManager.FontDefinitionFile.CODEC
