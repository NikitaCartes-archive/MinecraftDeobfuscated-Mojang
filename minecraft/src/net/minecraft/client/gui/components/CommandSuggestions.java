...
public class CommandSuggestions {
	...
	public void showSuggestions() {
		if (this.pendingSuggestions != null && this.pendingSuggestions.isDone()) {
			...
			if (!suggestions.isEmpty()) {
				...
				this.suggestions = new CommandSuggestions.SuggestionsList(j, k, i, suggestions);
			}
		}
	}
	...
	public void updateCommandInfo() {
		...
		if (j < i || this.suggestions != null && this.keepSuggestions) {
			int k = getLastWordIndex(string);
			Collection<String> collection = this.minecraft.player.connection.getSuggestionsProvider().getOnlinePlayerNames();
			this.pendingSuggestions = SharedSuggestionProvider.suggest(collection, new SuggestionsBuilder(string, k));
		} else {
