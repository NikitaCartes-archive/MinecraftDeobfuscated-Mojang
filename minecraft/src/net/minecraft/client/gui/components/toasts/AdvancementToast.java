...
public class AdvancementToast implements Toast {
	...
	public Toast.Visibility render(ToastComponent toastComponent, long l) {
		...
		if (displayInfo != null) {
			...
			if (list.size() == 1) {
				toastComponent.getMinecraft().font.draw(I18n.get("advancements.toast." + displayInfo.getFrame().getName()), 30.0F, 7.0F, i | 0xFF000000);
				...
				if (l < 1500L) {
					...
					toastComponent.getMinecraft().font.draw(I18n.get("advancements.toast." + displayInfo.getFrame().getName()), 30.0F, 11.0F, i | k);
				} else {
					int k = Mth.floor(Mth.clamp((float)(l - 1500L) / 300.0F, 0.0F, 1.0F) * 252.0F) << 24 | 67108864;
					int m = 16 - list.size() * 9 / 2;

					for (String string : list) {
						toastComponent.getMinecraft().font.draw(string, 30.0F, (float)m, 16777215 | k);
						m += 9;
					}
				}
			}
