...
public class WardenModel<T extends Warden> extends HierarchicalModel<T> {
	...
	public void setupAnim(T warden, float f, float g, float h, float i, float j) {
		...
		float t = warden.getEarAnimation(k) * (float)(Math.cos((double)h * 2.25) * Math.PI * 0.1F);
		this.leftTendril.xRot = t;
		this.rightTendril.xRot = -t;
		long u = Util.getMillis();
		this.animate(warden.attackAnimationState, WardenAnimation.WARDEN_ATTACK, u);
		this.animate(warden.diggingAnimationState, WardenAnimation.WARDEN_DIG, u);
		this.animate(warden.emergeAnimationState, WardenAnimation.WARDEN_EMERGE, u);
		this.animate(warden.roarAnimationState, WardenAnimation.WARDEN_ROAR, u);
		this.animate(warden.sniffAnimationState, WardenAnimation.WARDEN_SNIFF, u);
	}
