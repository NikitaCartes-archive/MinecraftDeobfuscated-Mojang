...
public class WardenModel<T extends Warden> extends HierarchicalModel<T> {
	...
	public void setupAnim(T warden, float f, float g, float h, float i, float j) {
		...
		this.head.xRot = this.head.xRot + 1.2F * Mth.cos(n + (float) (Math.PI / 2)) * s;
		...
		float t = warden.getEarEmissiveAlpha(k, h) * (float)(Math.cos((double)h * 2.25) * Math.PI * 0.1F);
		this.leftTendril.xRot = t;
		this.rightTendril.xRot = -t;
		int u = warden.getAttackAnimationTick();
		if (u > 0) {
			float v = 1.0F - (Mth.triangleWave((float)u - k, 15.0F) / 2.0F + 0.5F);
			float w = 0.3F * Mth.cos(((float)u - k) * 0.35F);
			float x = (Math.max(0.5F, v) - 0.5F) * 2.0F;
			float y = -1.2707963F * v;
			float z = -4.5F * v;
			float aa = 6.0F * v;
			this.head.xRot += 1.5F * v;
			this.head.zRot += w;
			this.leftArm.xRot += y;
			this.leftArm.yRot += 1.25F * x;
			this.leftArm.zRot += w;
			this.leftArm.z += z;
			this.leftArm.x += -aa;
			this.leftArm.y += 15.0F * v;
			this.rightArm.xRot += y;
			this.rightArm.yRot += -0.85F * x;
			this.rightArm.zRot += w;
			this.rightArm.z += z;
			this.rightArm.x += aa;
			this.rightArm.y += 5.0F * v;
		}

		long ab = Util.getMillis();
		this.animate(warden.roarAnimationState, WardenAnimation.WARDEN_ROAR, ab);
		this.animate(warden.sniffAnimationState, WardenAnimation.WARDEN_SNIFF, ab);
		this.animate(warden.emergeAnimationState, WardenAnimation.WARDEN_EMERGE, ab);
		this.animate(warden.diggingAnimationState, WardenAnimation.WARDEN_DIG, ab);
	}
