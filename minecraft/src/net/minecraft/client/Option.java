...
public abstract class Option {
	...
	public static final ProgressOption SIMULATION_DISTANCE = new ProgressOption(
		...
		16.0,
		1.0F,
		options -> (double)options.simulationDistance,
		(options, double_) -> options.simulationDistance = double_.intValue(),
		(options, progressOption) -> {
			double d = progressOption.get(options);
			return progressOption.genericValueLabel(new TranslatableComponent("options.chunks", (int)d));
		}
	...
	public static final CycleOption<String> AUDIO_DEVICE = CycleOption.create(
		"options.audioDevice",
		(Supplier<List<String>>)(() -> Stream.concat(Stream.of(""), Minecraft.getInstance().getSoundManager().getAvailableSoundDevices().stream()).toList()),
		string -> {
			if ("".equals(string)) {
	...
	public static final CycleOption<Boolean> ALLOW_SERVER_LISTING = CycleOption.createOnOff(
		"options.allowServerListing", ALLOW_SERVER_LISTING_TOOLTIP, options -> options.allowServerListing, (options, option, boolean_) -> {
	...
	public static final CycleOption<Boolean> DIRECTIONAL_AUDIO = CycleOption.createOnOff(
			"options.directionalAudio", options -> options.directionalAudio, (options, option, boolean_) -> {
		...
		.setTooltip(
			minecraft -> boolean_ -> boolean_ ? minecraft.font.split(DIRECTIONAL_AUDIO_TOOLTIP_ON, 200) : minecraft.font.split(DIRECTIONAL_AUDIO_TOOLTIP_OFF, 200)
		);
	...
	public static final CycleOption<Boolean> AUTOSAVE_INDICATOR = CycleOption.createOnOff(
		"options.autosaveIndicator", options -> options.showAutosaveIndicator, (options, option, boolean_) -> options.showAutosaveIndicator = boolean_
	);
