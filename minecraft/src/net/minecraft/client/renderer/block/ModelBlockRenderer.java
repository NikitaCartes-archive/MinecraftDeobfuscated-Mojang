...
public class ModelBlockRenderer {
	...
	public void tesselateWithAO(
		...
		for (Direction direction : DIRECTIONS) {
			...
			if (!list.isEmpty()) {
				...
				if (!bl || Block.shouldRenderFace(blockState, blockAndTintGetter.getBlockState(mutableBlockPos), direction)) {
	...
	static class AmbientOcclusionFace {
		...
		public void calculate(
			...
			if (bitSet.get(0) || !blockState11.isSolidRender()) {
	...
	protected static enum SizeInfo {
		...
		private SizeInfo(final Direction direction, final boolean bl) {
