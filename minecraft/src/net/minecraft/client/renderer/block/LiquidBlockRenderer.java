	...
	public boolean tesselate(BlockAndTintGetter blockAndTintGetter, BlockPos blockPos, VertexConsumer vertexConsumer, BlockState blockState, FluidState fluidState) {
		...
		if (!bl2 && !bl3 && !bl7 && !bl6 && !bl4 && !bl5) {
			...
			if (bl3) {
				...
				float aex = j * g;
				float agx = j * h;
				this.vertex(vertexConsumer, d, e + (double)y, w + 1.0, acx, aex, agx, zx, afx, aq);
			...
			for (Direction direction : Direction.Plane.HORIZONTAL) {
				...
				if (bl9
					...
					float aw = textureAtlasSprite2.getV((double)((1.0F - afx) * 16.0F * 0.5F));
					float ax = textureAtlasSprite2.getV((double)((1.0F - aax) * 16.0F * 0.5F));
					float ay = textureAtlasSprite2.getV(8.0);
					float az = direction.getAxis() == Direction.Axis.Z ? l : m;
					float ba = k * az * f;
					float bb = k * az * g;
					float bc = k * az * h;
					this.vertex(vertexConsumer, as, e + (double)afx, au, ba, bb, bc, ao, aw, ar);
					this.vertex(vertexConsumer, at, e + (double)aax, av, ba, bb, bc, ap, ax, ar);
