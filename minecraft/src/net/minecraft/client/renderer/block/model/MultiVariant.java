...
public class MultiVariant implements UnbakedModel {
	...
	public Collection<ResourceLocation> getTextures(Function<ResourceLocation, UnbakedModel> function, Set<String> set) {
		return (Collection<ResourceLocation>)this.getVariants()
			.stream()
			.map(Variant::getModelLocation)
			.distinct()
			.flatMap(resourceLocation -> ((UnbakedModel)function.apply(resourceLocation)).getTextures(function, set).stream())
