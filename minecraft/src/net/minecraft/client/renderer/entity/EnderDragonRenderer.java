...
public class EnderDragonRenderer extends EntityRenderer<EnderDragon> {
	...
	public static class DragonModel extends EntityModel<EnderDragon> {
		...
		public void renderToBuffer(PoseStack poseStack, VertexConsumer vertexConsumer, int i, int j, float f, float g, float h) {
			...
			this.jaw.xRot = (float)(Math.sin((double)(k * (float) (Math.PI * 2))) + 1.0) * 0.2F;
			float l = (float)(Math.sin((double)(k * (float) (Math.PI * 2) - 1.0F)) + 1.0);
			l = (l * l + l * 2.0F) * 0.05F;
			poseStack.translate(0.0, (double)(l - 2.0F), -3.0);
			...
			for (int t = 0; t < 5; t++) {
				...
				float u = (float)Math.cos((double)((float)t * 0.45F + s)) * 0.15F;
				this.neck.yRot = Mth.rotWrap(es[0] - ds[0]) * (float) (Math.PI / 180.0) * 1.5F;
				this.neck.xRot = u + this.entity.getHeadPartYOffset(t, ds, es) * (float) (Math.PI / 180.0) * 1.5F * 5.0F;
				this.neck.zRot = -Mth.rotWrap(es[0] - (double)r) * (float) (Math.PI / 180.0) * 1.5F;
				this.neck.y = n;
				this.neck.z = o;
				this.neck.x = m;
				n = (float)((double)n + Math.sin((double)this.neck.xRot) * 10.0);
				o = (float)((double)o - Math.cos((double)this.neck.yRot) * Math.cos((double)this.neck.xRot) * 10.0);
				m = (float)((double)m - Math.sin((double)this.neck.yRot) * Math.cos((double)this.neck.xRot) * 10.0);
			...
			this.head.zRot = -Mth.rotWrap(fs[0] - (double)r) * (float) (Math.PI / 180.0);
			...
			this.leftWing.xRot = 0.125F - (float)Math.cos((double)v) * 0.2F;
			this.leftWing.yRot = -0.25F;
			this.leftWing.zRot = -((float)(Math.sin((double)v) + 0.125)) * 0.8F;
			this.leftWingTip.zRot = (float)(Math.sin((double)(v + 2.0F)) + 0.5) * 0.75F;
			...
			poseStack.popPose();
			float u = -((float)Math.sin((double)(k * (float) (Math.PI * 2)))) * 0.0F;
			s = k * (float) (Math.PI * 2);
			n = 10.0F;
			o = 60.0F;
			m = 0.0F;
			...
			for (int w = 0; w < 12; w++) {
				...
				u = (float)((double)u + Math.sin((double)((float)w * 0.45F + s)) * 0.05F);
				...
				this.neck.zRot = Mth.rotWrap(fs[0] - (double)r) * (float) (Math.PI / 180.0) * 1.5F;
				this.neck.y = n;
				this.neck.z = o;
				this.neck.x = m;
				n = (float)((double)n + Math.sin((double)this.neck.xRot) * 10.0);
