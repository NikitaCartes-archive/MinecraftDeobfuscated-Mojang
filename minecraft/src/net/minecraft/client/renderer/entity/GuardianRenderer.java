...
public class GuardianRenderer extends MobRenderer<Guardian, GuardianModel> {
	...
	public GuardianRenderer(EntityRendererProvider.Context context, ModelLayerLocation modelLayerLocation) {
	...
	public void render(Guardian guardian, float f, float g, PoseStack poseStack, MultiBufferSource multiBufferSource, int i) {
		...
		if (livingEntity != null) {
			...
			float m = (float)(vec33.length() + (bl ? 0.1 : 1.0));
			...
			float aq = -1.0F + k;
			float ar = m * 2.5F + aq;
			VertexConsumer vertexConsumer = multiBufferSource.getBuffer(RenderType.entityCutoutNoCull(bl ? TOXIFIN_BEAM_LOCATION : GUARDIAN_BEAM_LOCATION));
	...
	public ResourceLocation getTextureLocation(Guardian guardian) {
		return guardian.isToxic() ? TOXIFIN_LOCATION : GUARDIAN_LOCATION;
	}
}
