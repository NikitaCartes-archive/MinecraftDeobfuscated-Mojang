...
public class ArmorStandRenderer extends LivingEntityRenderer<ArmorStand, ArmorStandArmorModel> {
	...
	protected void setupRotations(ArmorStand armorStand, PoseStack poseStack, float f, float g, float h, float i) {
		poseStack.mulPose(Axis.YP.rotationDegrees(180.0F - g));
		float j = (float)(armorStand.level().getGameTime() - armorStand.lastHit) + h;
		if (j < 5.0F) {
			poseStack.mulPose(Axis.YP.rotationDegrees(Mth.sin(j / 1.5F * (float) Math.PI) * 3.0F));
		}
	}
