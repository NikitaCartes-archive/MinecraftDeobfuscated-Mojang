...
public class BreezeWindLayer extends RenderLayer<Breeze, BreezeModel<Breeze>> {
	...
	public BreezeWindLayer(RenderLayerParent<Breeze, BreezeModel<Breeze>> renderLayerParent, EntityModelSet entityModelSet, ResourceLocation resourceLocation) {
	...
	public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, Breeze breeze, float f, float g, float h, float j, float k, float l) {
		...
		VertexConsumer vertexConsumer = multiBufferSource.getBuffer(RenderType.breezeWind(this.getTextureLocation(breeze), this.xOffset(m) % 1.0F, 0.0F));
