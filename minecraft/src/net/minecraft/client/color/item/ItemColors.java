...
public class ItemColors {
	...
	public static ItemColors createDefault(BlockColors blockColors) {
		...
		itemColors.register(
			(itemStack, i) -> i > 0
					? -1
					: FastColor.ARGB32.opaque(((PotionContents)itemStack.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY)).getColor()),
			Items.POTION,
			Items.SPLASH_POTION,
		...
		itemColors.register(
			(itemStack, i) -> i == 0 ? -1 : FastColor.ARGB32.opaque(((MapItemColor)itemStack.getOrDefault(DataComponents.MAP_COLOR, MapItemColor.DEFAULT)).rgb()),
			Items.FILLED_MAP
		);
		return itemColors;
	}
