...
public class TagLoader<T> {
			...
			for(Resource resource : (List)entry.getValue()) {
				try {
					...
					try {
						...
						List<TagLoader.EntryWithSource> list = (List)map.computeIfAbsent(resourceLocation2, resourceLocationx -> new ArrayList());
	...
	public Map<ResourceLocation, Collection<T>> build(Map<ResourceLocation, List<TagLoader.EntryWithSource>> map) {
		final Map<ResourceLocation, Collection<T>> map2 = Maps.newHashMap();
