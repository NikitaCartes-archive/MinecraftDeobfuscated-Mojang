	...
	public static <T, C, P> Grammar<List<T>> createGrammar(ComponentPredicateParser.Context<T, C, P> context) {
		...
		dictionary.put(
			...
			scope -> {
				...
				((Optional)scope.getOrThrow(atom2)).ifPresent(builder::add);
		...
		dictionary.put(
			...
			scope -> (T)Optional.ofNullable(scope.getAny(atom4, atom5))
		);
		dictionary.put(atom3, StringReaderTerms.character('*'), scope -> (T)Unit.INSTANCE);
		...
		dictionary.put(atom6, Term.sequence(Term.named(atom7), Term.optional(Term.sequence(StringReaderTerms.character(','), Term.named(atom6)))), scope -> {
			...
			return (T)((List)Optional.ofNullable((List)scope.get(atom6)).map(list -> Util.copyAndAdd(object, list)).orElse(List.of(object)));
		});
		dictionary.put(atom7, Term.sequence(Term.named(atom8), Term.optional(Term.sequence(StringReaderTerms.character('|'), Term.named(atom7)))), scope -> {
			...
			return (T)((List)Optional.ofNullable((List)scope.get(atom7)).map(list -> Util.copyAndAdd(object, list)).orElse(List.of(object)));
		});
		dictionary.put(
		...
		dictionary.put(
			...
			(parseState, scope) -> {
				...
				try {
					...
					parseState.errorCollector().store(parseState.mark(), var9xx);
