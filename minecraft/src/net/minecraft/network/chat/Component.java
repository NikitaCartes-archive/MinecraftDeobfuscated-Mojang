...
public interface Component extends Message, FormattedText {
	...
	default String tryCollapseToString() {
		...
		if (var2 instanceof PlainTextContents plainTextContents && this.getSiblings().isEmpty() && this.getStyle().isEmpty()) {
	...
	public static class Serializer {
		...
		static MutableComponent deserialize(JsonElement jsonElement, HolderLookup.Provider provider) {
			return Util.getOrThrow(ComponentSerialization.CODEC.parse(provider.createSerializationContext(JsonOps.INSTANCE), jsonElement), JsonParseException::new);
		}
