...
public class ComponentSerialization {
	...
	public static Codec<Component> flatCodec(int i) {
		...
		return new Codec<Component>() {
			...
			private static <T> DynamicOps<JsonElement> asJsonOps(DynamicOps<T> dynamicOps) {
				return (DynamicOps<JsonElement>)(dynamicOps instanceof RegistryOps registryOps ? registryOps.withParent(JsonOps.INSTANCE) : JsonOps.INSTANCE);
			}
		};
	}
	...
	static class FuzzyCodec<T> extends MapCodec<T> {
		...
		public <S> RecordBuilder<S> encode(T object, DynamicOps<S> dynamicOps, RecordBuilder<S> recordBuilder) {
			MapEncoder<T> mapEncoder = (MapEncoder)this.encoderGetter.apply(object);
