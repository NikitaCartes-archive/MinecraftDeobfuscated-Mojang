...
implements RealmsWorldSlotButton.Listener {
    ...
    public void render(int i, int j, float f) {
        ...
        int m = this.fontWidth(RealmsConfigureWorldScreen.getLocalizedString("mco.configure.world.title"));
    ...
    private void fetchServerData(long l) {
        new Thread(() -> {
            ...
            try {
                ...
                Realms.setScreen(new RealmsGenericErrorScreen(realmsServiceException.getMessage(), (RealmsScreen)this.lastScreen));
    ...
    private void switchToEmptySlot(int i, RealmsServer realmsServer) {
        ...
        Realms.setScreen(new RealmsLongConfirmationScreen((bl, j) -> {
            if (bl) {
                RealmsResetWorldScreen realmsResetWorldScreen = new RealmsResetWorldScreen(this, realmsServer, this.getNewScreen(), RealmsConfigureWorldScreen.getLocalizedString("mco.configure.world.switch.slot"), RealmsConfigureWorldScreen.getLocalizedString("mco.configure.world.switch.slot.subtitle"), 0xA0A0A0, RealmsConfigureWorldScreen.getLocalizedString("gui.cancel"));
    ...
    private void drawExpiring(int i, int j, int k, int l, int m) {
        ...
        RenderSystem.popMatrix();
        if (k >= i && k <= i + 9 && l >= j && l <= j + 27) {
            this.toolTip = m <= 0 ? RealmsConfigureWorldScreen.getLocalizedString("mco.selectServer.expires.soon") : (m == 1 ? RealmsConfigureWorldScreen.getLocalizedString("mco.selectServer.expires.day") : RealmsConfigureWorldScreen.getLocalizedString("mco.selectServer.expires.days", m));
        }
    }
    ...
    private void switchSlot(long l, int i2) {
        RealmsConfigureWorldScreen realmsConfigureWorldScreen = this.getNewScreen();
        RealmsTasks.SwitchSlotTask switchSlotTask = new RealmsTasks.SwitchSlotTask(l, i2, (bl, i) -> Realms.setScreen(realmsConfigureWorldScreen), 11);
