...
public class UpdateOneTwentyRegistries {
    private static final RegistrySetBuilder BUILDER = new RegistrySetBuilder().add(Registries.TRIM_MATERIAL, TrimMaterials::nextUpdate).add(Registries.TRIM_PATTERN, TrimPatterns::nextUpdate).add(Registries.BIOME, BiomeData::nextUpdate).add(Registries.WORLD_PRESET, WorldPresets::nextUpdate);
    ...
    public static CompletableFuture<HolderLookup.Provider> createLookup(CompletableFuture<HolderLookup.Provider> completableFuture) {
        return completableFuture.thenApply(provider -> {
            ...
            HolderLookup.Provider provider2 = BUILDER.buildPatch(frozen, (HolderLookup.Provider)provider);
