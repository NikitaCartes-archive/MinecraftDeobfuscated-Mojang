...
public final class ModifiedGravellyMountainsBiome
extends Biome {
    public ModifiedGravellyMountainsBiome() {
        super(new Biome.BiomeBuilder().surfaceBuilder(SurfaceBuilders.GRAVELLY_MOUNTAIN).precipitation(Biome.Precipitation.RAIN).biomeCategory(Biome.BiomeCategory.EXTREME_HILLS).depth(1.0f).scale(0.5f).temperature(0.2f).downfall(0.3f).specialEffects(new BiomeSpecialEffects.Builder().waterColor(4159204).waterFogColor(329011).fogColor(12638463).ambientMoodSound(AmbientMoodSettings.LEGACY_CAVE_SETTINGS).build()).parent("wooded_mountains"));
