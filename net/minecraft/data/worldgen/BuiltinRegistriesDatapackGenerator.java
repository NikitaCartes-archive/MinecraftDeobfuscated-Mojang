...
implements DataProvider {
    ...
    public CompletableFuture<?> run(CachedOutput cachedOutput) {
        ...
        return CompletableFuture.allOf((CompletableFuture[])RegistryDataLoader.WORLDGEN_REGISTRIES.stream().map(registryData -> this.dumpRegistryCap(cachedOutput, registryAccess, (DynamicOps<JsonElement>)dynamicOps, (RegistryDataLoader.RegistryData)registryData)).toArray(CompletableFuture[]::new));
    }

    private <T> CompletableFuture<?> dumpRegistryCap(CachedOutput cachedOutput, RegistryAccess registryAccess, DynamicOps<JsonElement> dynamicOps, RegistryDataLoader.RegistryData<T> registryData) {
        ...
        return CompletableFuture.allOf((CompletableFuture[])registry.entrySet().stream().map(entry -> BuiltinRegistriesDatapackGenerator.dumpValue(pathProvider.json(((ResourceKey)entry.getKey()).location()), cachedOutput, dynamicOps, registryData.elementCodec(), entry.getValue())).toArray(CompletableFuture[]::new));
    }
