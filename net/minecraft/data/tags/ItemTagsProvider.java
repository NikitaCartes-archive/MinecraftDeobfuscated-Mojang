...
extends IntrinsicHolderTagsProvider<Item> {
    ...
    public ItemTagsProvider(PackOutput packOutput, CompletableFuture<HolderLookup.Provider> completableFuture, CompletableFuture<TagsProvider.TagLookup<Block>> completableFuture2) {
        super(packOutput, Registries.ITEM, completableFuture, (T item) -> item.builtInRegistryHolder().key());
    ...
    protected CompletableFuture<HolderLookup.Provider> contentsProvider() {
        return super.contentsProvider().thenCombineAsync(this.blockTags, (provider, tagLookup) -> {
            this.tagsToCopy.forEach((tagKey, tagKey2) -> {
                ...
                Optional optional = (Optional)tagLookup.apply(tagKey);
