...
public class ResetChunksCommand {
    ...
    private static int resetChunks(CommandSourceStack commandSourceStack, int i) {
        ...
        for (ChunkStatus chunkStatus : ImmutableList.of(ChunkStatus.BIOMES, ChunkStatus.NOISE, ChunkStatus.SURFACE, ChunkStatus.CARVERS, ChunkStatus.LIQUID_CARVERS, ChunkStatus.FEATURES)) {
            ...
            for (int o = chunkPos.z - i; o <= chunkPos.z + i; ++o) {
                for (int p = chunkPos.x - i; p <= chunkPos.x + i; ++p) {
                    ...
                    completableFuture = completableFuture.thenComposeAsync(unit -> chunkStatus.generate(processorMailbox::tell, serverLevel, serverLevel.getChunkSource().getGenerator(), serverLevel.getStructureManager(), serverChunkCache.getLightEngine(), chunkAccess -> {
        ...
        commandSourceStack.sendSuccess(new TextComponent(String.format("%d chunks have been reset. This took %d ms for %d chunks, or %02f ms per chunk", m, n, m, Float.valueOf((float)n / (float)m))), true);
        return 1;
    }
}
