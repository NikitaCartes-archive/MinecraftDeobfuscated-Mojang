...
public class WardenSpawnTrackerCommand {
    public static void register(CommandDispatcher<CommandSourceStack> commandDispatcher) {
        commandDispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("warden_spawn_tracker").requires(commandSourceStack -> commandSourceStack.hasPermission(2))).then(Commands.literal("clear").executes(commandContext -> WardenSpawnTrackerCommand.resetTracker((CommandSourceStack)commandContext.getSource(), ImmutableList.of(((CommandSourceStack)commandContext.getSource()).getPlayerOrException()))))).then(Commands.literal("set").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("warning_level", IntegerArgumentType.integer(0, 4)).executes(commandContext -> WardenSpawnTrackerCommand.setWarningLevel((CommandSourceStack)commandContext.getSource(), ImmutableList.of(((CommandSourceStack)commandContext.getSource()).getPlayerOrException()), IntegerArgumentType.getInteger(commandContext, "warning_level"))))));
    }
