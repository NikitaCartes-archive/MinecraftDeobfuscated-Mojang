...
public class EntitySelectorParser {
    ...
    private Predicate<Entity> createRotationPredicate(WrappedMinMaxBounds wrappedMinMaxBounds, ToDoubleFunction<Entity> toDoubleFunction) {
        ...
        double e = Mth.wrapDegrees(wrappedMinMaxBounds.getMax() == null ? 359.0f : wrappedMinMaxBounds.getMax().floatValue());
        return entity -> {
            double f = Mth.wrapDegrees(toDoubleFunction.applyAsDouble((Entity)entity));
            if (d > e) {
                return f >= d || f <= e;
            }
            return f >= d && f <= e;
