...
public record SignedMessageBody(String content, Instant timeStamp, long salt, LastSeenMessages lastSeen) {
    ...
    public Packed pack(MessageSignature.Packer packer) {
        return new Packed(this.content, this.timeStamp, this.salt, this.lastSeen.pack(packer));
    }
    ...
    public record Packed(String content, Instant timeStamp, long salt, LastSeenMessages.Packed lastSeen) {
        ...
        public Optional<SignedMessageBody> unpack(MessageSignature.Unpacker unpacker) {
            return this.lastSeen.unpack(unpacker).map(lastSeenMessages -> new SignedMessageBody(this.content, this.timeStamp, this.salt, (LastSeenMessages)lastSeenMessages));
        }
    }
}
