...
public record ClientboundPlayerChatPacket(Component content, ChatType type, ChatSender sender, Instant timeStamp, Crypt.SaltSignaturePair saltSignature) implements Packet<ClientGamePacketListener>
{
    ...
    public ClientboundPlayerChatPacket(FriendlyByteBuf friendlyByteBuf) {
        this(friendlyByteBuf.readComponent(), ChatType.getForIndex(friendlyByteBuf.readByte()), new ChatSender(friendlyByteBuf), Instant.ofEpochSecond(friendlyByteBuf.readLong()), new Crypt.SaltSignaturePair(friendlyByteBuf));
    }
