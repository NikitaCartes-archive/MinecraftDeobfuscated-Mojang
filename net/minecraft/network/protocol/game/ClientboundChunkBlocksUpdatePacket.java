...
public class ClientboundChunkBlocksUpdatePacket
implements Packet<ClientGamePacketListener> {
    ...
    public ClientboundChunkBlocksUpdatePacket(int i, short[] ss, LevelChunk levelChunk) {
        ...
        this.updates = new BlockUpdate[i];
        for (int j = 0; j < this.updates.length; ++j) {
            this.updates[j] = new BlockUpdate(ss[j], levelChunk);
        }
    }
    ...
    public void read(FriendlyByteBuf friendlyByteBuf) throws IOException {
        ...
        this.updates = new BlockUpdate[friendlyByteBuf.readVarInt()];
        for (int i = 0; i < this.updates.length; ++i) {
            this.updates[i] = new BlockUpdate(friendlyByteBuf.readShort(), Block.BLOCK_STATE_REGISTRY.byId(friendlyByteBuf.readVarInt()));
        }
    }
    ...
    public void write(FriendlyByteBuf friendlyByteBuf) throws IOException {
        ...
        for (BlockUpdate blockUpdate : this.updates) {
