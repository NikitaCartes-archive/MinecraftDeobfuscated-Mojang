...
public enum ConnectionProtocol {
    ...
    protected ConnectionProtocol addPacket(PacketFlow packetFlow, Class<? extends Packet<?>> class_) {
        BiMap<Integer, Class<Packet<?>>> biMap = this.packets.get((Object)packetFlow);
        if (biMap == null) {
            biMap = HashBiMap.create();
            this.packets.put(packetFlow, biMap);
        }
        if (biMap.containsValue(class_)) {
            String string = (Object)((Object)packetFlow) + " packet " + class_ + " is already known to ID " + biMap.inverse().get(class_);
    ...
    public Integer getPacketId(PacketFlow packetFlow, Packet<?> packet) throws Exception {
        return (Integer)this.packets.get((Object)packetFlow).inverse().get(packet.getClass());
    }

    @Nullable
    public Packet<?> createPacket(PacketFlow packetFlow, int i) throws IllegalAccessException, InstantiationException {
        Class class_ = (Class)this.packets.get((Object)packetFlow).get(i);
        if (class_ == null) {
            return null;
        }
    ...
    static {
        ...
        for (ConnectionProtocol connectionProtocol : ConnectionProtocol.values()) {
            ...
            for (PacketFlow packetFlow : connectionProtocol.packets.keySet()) {
                for (Class class_ : connectionProtocol.packets.get((Object)packetFlow).values()) {
                    if (PROTOCOL_BY_PACKET.containsKey(class_) && PROTOCOL_BY_PACKET.get(class_) != connectionProtocol) {
                        throw new Error("Packet " + class_ + " is already assigned to protocol " + (Object)((Object)PROTOCOL_BY_PACKET.get(class_)) + " - can't reassign to " + (Object)((Object)connectionProtocol));
                    }
                    try {
                        class_.newInstance();
