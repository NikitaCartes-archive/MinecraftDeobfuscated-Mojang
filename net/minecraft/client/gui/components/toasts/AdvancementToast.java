...
implements Toast {
    ...
    public Toast.Visibility render(ToastComponent toastComponent, long l) {
        ...
        if (displayInfo != null) {
            int i;
            List<String> list = toastComponent.getMinecraft().font.split(displayInfo.getTitle().getColoredString(), 125);
            int n = i = displayInfo.getFrame() == FrameType.CHALLENGE ? 0xFF88FF : 0xFFFF00;
            if (list.size() == 1) {
                toastComponent.getMinecraft().font.draw(I18n.get("advancements.toast." + displayInfo.getFrame().getName(), new Object[0]), 30.0f, 7.0f, i | 0xFF000000);
                toastComponent.getMinecraft().font.draw(displayInfo.getTitle().getColoredString(), 30.0f, 18.0f, -1);
            } else {
                int j = 1500;
                float f = 300.0f;
                if (l < 1500L) {
                    int k = Mth.floor(Mth.clamp((float)(1500L - l) / 300.0f, 0.0f, 1.0f) * 255.0f) << 24 | 0x4000000;
                    toastComponent.getMinecraft().font.draw(I18n.get("advancements.toast." + displayInfo.getFrame().getName(), new Object[0]), 30.0f, 11.0f, i | k);
                } else {
                    ...
                    for (String string : list) {
                        toastComponent.getMinecraft().font.draw(string, 30.0f, m, 0xFFFFFF | k);
