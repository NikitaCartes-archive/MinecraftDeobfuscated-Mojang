...
extends Screen {
    ...
    protected void init() {
        ...
        if (serverData != null && (chatPreview = serverData.getChatPreview()) != null && chatPreview.showToast()) {
    ...
    private void updateChatPreview(String string) {
        ...
        if (this.sendsChatPreviewRequests() && !string2.startsWith("/")) {
            this.chatPreview.request(string2);
        } else {
    ...
    private boolean sendsChatPreviewRequests() {
        ...
        if (this.minecraft.options.chatPreview().get().booleanValue()) {
            ServerData serverData = this.minecraft.getCurrentServer();
    ...
    public void renderChatPreview(PoseStack poseStack) {
        int i = (int)(255.0 * (this.minecraft.options.chatOpacity().get() * (double)0.9f + (double)0.1f));
        ...
        poseStack.pushPose();
        poseStack.translate(this.chatPreviewLeft(), this.chatPreviewTop(l), 0.0);
        ChatScreen.fill(poseStack, 0, 0, k, l, j << 24);
        poseStack.translate(2.0, 2.0, 0.0);
        for (int m = 0; m < list.size(); ++m) {
            FormattedCharSequence formattedCharSequence = list.get(m);
            this.minecraft.font.drawShadow(poseStack, formattedCharSequence, 0.0f, (float)(m * this.font.lineHeight), i << 24 | 0xFFFFFF);
        }
        poseStack.popPose();
    ...
    private List<FormattedCharSequence> peekChatPreview() {
        ...
        return List.of(PREVIEW_HINT.getVisualOrderText());
    }
