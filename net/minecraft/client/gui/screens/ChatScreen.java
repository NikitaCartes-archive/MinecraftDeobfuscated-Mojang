...
extends Screen {
    ...
    protected void init() {
        ...
        ChatPreviewStatus chatPreviewStatus = this.chatPreviewStatus = serverData != null && !serverData.previewsChat() ? ChatPreviewStatus.OFF : this.minecraft.options.chatPreview().get();
        if (serverData != null && this.chatPreviewStatus != ChatPreviewStatus.OFF && (chatPreview = serverData.getChatPreview()) != null && serverData.previewsChat() && chatPreview.showToast()) {
    ...
    private boolean sendsChatPreviewRequests() {
        ...
        if (this.chatPreviewStatus != ChatPreviewStatus.OFF) {
            ServerData serverData = this.minecraft.getCurrentServer();
    ...
    public void renderChatPreview(PoseStack poseStack, Component component, float f, boolean bl) {
        int n;
        int i = (int)(255.0 * (this.minecraft.options.chatOpacity().get() * (double)0.9f + (double)0.1f) * (double)f);
        ...
        poseStack.pushPose();
        poseStack.translate(this.chatPreviewLeft(), m, 0.0);
        ChatScreen.fill(poseStack, 0, 0, k, l, j << 24);
        if (i > 0) {
            poseStack.translate(2.0, 2.0, 0.0);
            for (n = 0; n < list.size(); ++n) {
                FormattedCharSequence formattedCharSequence = list.get(n);
                int o = n * this.font.lineHeight;
                this.renderChatPreviewHighlights(poseStack, formattedCharSequence, o, i);
                this.font.drawShadow(poseStack, formattedCharSequence, 0.0f, (float)o, i << 24 | 0xFFFFFF);
            }
        }
        poseStack.popPose();
    ...
    private void renderChatPreviewHighlights(PoseStack poseStack, FormattedCharSequence formattedCharSequence, int i, int j) {
        ...
        for (StringSplitter.Span span : this.font.getSplitter().findSpans(formattedCharSequence, predicate)) {
            ...
            ChatScreen.fill(poseStack, m, i, n, k, l);
        }
    }
    ...
    private Style getChatPreviewStyleAt(double d, double e) {
        ...
        int l = (Mth.floor(e) - k) / this.font.lineHeight;
        if (l >= 0 && l < list.size()) {
    ...
    private int chatPreviewHeight(List<FormattedCharSequence> list) {
        return Math.max(list.size(), 1) * this.font.lineHeight + 4;
    }
