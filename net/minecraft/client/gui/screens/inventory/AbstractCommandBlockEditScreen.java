...
extends Screen {
    ...
    public boolean keyPressed(int i, int j, int k) {
        ...
        if (this.getFocused() == this.commandEdit && i == 258) {
    ...
    public boolean mouseScrolled(double d, double e, double f) {
        if (this.suggestions != null && this.suggestions.mouseScrolled(Mth.clamp(f, -1.0, 1.0))) {
            return true;
        }
    ...
    protected void updateCommandInfo() {
        ...
        if (!((j = this.commandEdit.getCursorPosition()) < i || this.suggestions != null && this.keepSuggestions)) {
    ...
    private void updateUsageInfo() {
        if (this.pendingSuggestions.join().isEmpty() && !this.currentParse.getExceptions().isEmpty() && this.commandEdit.getCursorPosition() == this.commandEdit.getValue().length()) {
            int i = 0;
    ...
    private void fillNodeUsage(ChatFormatting chatFormatting) {
        ...
        if (!list.isEmpty()) {
            ...
            this.commandUsagePosition = Mth.clamp(this.commandEdit.getScreenX(suggestionContext.startPos), 0, this.commandEdit.getScreenX(0) + this.commandEdit.getInnerWidth() - i);
    ...
    public void render(int i, int j, float f) {
        ...
        if (this.suggestions != null) {
            ...
            for (String string : this.commandUsage) {
                AbstractCommandBlockEditScreen.fill(this.commandUsagePosition - 1, 72 + 12 * k, this.commandUsagePosition + this.commandUsageWidth + 1, 84 + 12 * k, Integer.MIN_VALUE);
                this.font.drawShadow(string, this.commandUsagePosition, 74 + 12 * k, -1);
                ++k;
            }
        }
    }
    ...
    public void showSuggestions() {
        ...
        if (this.pendingSuggestions != null && this.pendingSuggestions.isDone() && !(suggestions = this.pendingSuggestions.join()).isEmpty()) {
            ...
            this.suggestions = new SuggestionsList(j, 72, i, suggestions);
        }
    }
    ...
    class SuggestionsList {
        ...
        public void render(int i, int j) {
            ...
            if (bl3) {
                ...
                if (bl) {
                    for (n = 0; n < this.rect.getWidth(); ++n) {
                        if (n % 2 != 0) continue;
                ...
                if (bl2) {
                    for (n = 0; n < this.rect.getWidth(); ++n) {
                        if (n % 2 != 0) continue;
                        GuiComponent.fill(this.rect.getX() + n, this.rect.getY() + this.rect.getHeight(), this.rect.getX() + n + 1, this.rect.getY() + this.rect.getHeight() + 1, -1);
                    }
                }
            }
            boolean bl52 = false;
            for (int o = 0; o < k; ++o) {
                Suggestion suggestion = this.suggestions.getList().get(o + this.offset);
                ...
                AbstractCommandBlockEditScreen.this.font.drawShadow(suggestion.getText(), this.rect.getX() + 1, this.rect.getY() + 2 + 12 * o, o + this.offset == this.current ? -256 : -5592406);
            }
        ...
        public void select(int i) {
            ...
            AbstractCommandBlockEditScreen.this.commandEdit.setSuggestion(AbstractCommandBlockEditScreen.calculateSuggestionSuffix(AbstractCommandBlockEditScreen.this.commandEdit.getValue(), suggestion.apply(this.originalContents)));
        }
