    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, double d, double e, double f) {
        ...
        for (TrackedListener trackedListener2 : this.trackedListeners) {
            trackedListener2.getPosition(level).ifPresent(vec3 -> {
                ...
                LevelRenderer.renderVoxelShape(poseStack, vertexConsumer, Shapes.create(new AABB(g, h, i, j, k, l)), -d, -e, -f, 1.0f, 1.0f, 0.0f, 0.35f);
            });
        }
        VertexConsumer vertexConsumer2 = multiBufferSource.getBuffer(RenderType.debugFilledBox());
        for (TrackedListener trackedListener2 : this.trackedListeners) {
            trackedListener2.getPosition(level).ifPresent(vec3 -> LevelRenderer.addChainedFilledBoxVertices(poseStack, vertexConsumer2, vec3.x() - 0.25 - d, vec3.y() - e, vec3.z() - 0.25 - f, vec3.x() + 0.25 - d, vec3.y() - e + 1.0, vec3.z() + 0.25 - f, 1.0f, 1.0f, 0.0f, 0.35f));
        }
        for (TrackedListener trackedListener2 : this.trackedListeners) {
            trackedListener2.getPosition(level).ifPresent(vec3 -> {
        ...
        for (TrackedGameEvent trackedGameEvent : this.trackedGameEvents) {
            ...
            GameEventListenerRenderer.renderFilledBox(poseStack, multiBufferSource, new AABB(h, i, j, k, l, m), 1.0f, 1.0f, 1.0f, 0.2f);
            DebugRenderer.renderFloatingText(poseStack, multiBufferSource, trackedGameEvent.gameEvent.getName(), vec322.x, vec322.y + (double)0.85f, vec322.z, -7564911, 0.0075f);
        }
    }
