...
implements DebugRenderer.SimpleDebugRenderer {
    ...
    private static void doRenderPath(Path path, float f, boolean bl, boolean bl2, double d, double e, double g) {
        ...
        if (PathfindingRenderer.distanceToCamera(blockPos, d, e, g) <= 40.0f) {
            ...
            for (i = 0; i < path.getSize(); ++i) {
                ...
                if (!(PathfindingRenderer.distanceToCamera(node.asBlockPos(), d, e, g) <= 40.0f)) continue;
        ...
        if (bl) {
            for (Node node2 : path.getClosedSet()) {
                if (!(PathfindingRenderer.distanceToCamera(node2.asBlockPos(), d, e, g) <= 40.0f)) continue;
    ...
    public static void renderPathLine(Path path, double d, double e, double f) {
        ...
        for (int i = 0; i < path.getSize(); ++i) {
            Node node = path.get(i);
            if (PathfindingRenderer.distanceToCamera(node.asBlockPos(), d, e, f) > 40.0f) continue;
