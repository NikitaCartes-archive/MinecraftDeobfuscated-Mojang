...
public class LiquidBlockRenderer {
    ...
    public boolean tesselate(BlockAndTintGetter blockAndTintGetter, BlockPos blockPos, VertexConsumer vertexConsumer, BlockState blockState, FluidState fluidState) {
        ...
        if (bl2 && !LiquidBlockRenderer.isFaceOccludedByNeighbor(blockAndTintGetter, blockPos, Direction.UP, Math.min(Math.min(p, r), Math.min(q, o)), blockState3)) {
            ...
            ah = (aa + ac + ae + ag) / 4.0f;
            ai = (float)textureAtlasSprites[0].getWidth() / (textureAtlasSprites[0].getU1() - textureAtlasSprites[0].getU0());
            aj = (float)textureAtlasSprites[0].getHeight() / (textureAtlasSprites[0].getV1() - textureAtlasSprites[0].getV0());
        ...
        for (Direction direction : Direction.Plane.HORIZONTAL) {
            ...
            ao = textureAtlasSprite2.getU(0.0);
            ap = textureAtlasSprite2.getU(8.0);
            float aw = textureAtlasSprite2.getV((1.0f - af) * 16.0f * 0.5f);
            float ax = textureAtlasSprite2.getV((1.0f - aa) * 16.0f * 0.5f);
