...
public abstract class EntityRenderer<T extends Entity> {
    ...
    protected int getTeamColor(T entity) {
        PlayerTeam playerTeam = (PlayerTeam)((Entity)entity).getTeam();
        if (playerTeam != null && playerTeam.getColor().getColor() != null) {
    ...
    protected void renderName(T entity, double d, double e, double f) {
        ...
        this.renderNameTag(entity, ((Entity)entity).getDisplayName().getColoredString(), d, e, f, 64);
    }
    ...
    private void renderFlame(Entity entity, double d, double e, double f, float g) {
        ...
        while (k > 0.0f) {
            ...
            bufferBuilder.vertex(i - 0.0f, 0.0f - l, m).uv(q, r).endVertex();
            bufferBuilder.vertex(-i - 0.0f, 0.0f - l, m).uv(o, r).endVertex();
            bufferBuilder.vertex(-i - 0.0f, 1.4f - l, m).uv(o, p).endVertex();
            bufferBuilder.vertex(i - 0.0f, 1.4f - l, m).uv(q, p).endVertex();
            k -= 0.45f;
            l -= 0.45f;
            i *= 0.9f;
            m += 0.03f;
    ...
    private void renderShadow(Entity entity, double d, double e, double f, float g, float h) {
        ...
        for (BlockPos blockPos : BlockPos.betweenClosed(new BlockPos(m, o, q), new BlockPos(n, p, r))) {
            ...
            if (blockState.getRenderShape() == RenderShape.INVISIBLE || levelReader.getMaxLocalRawBrightness(blockPos) <= 3) continue;
    ...
    private void renderBlockShadow(BlockState blockState, LevelReader levelReader, BlockPos blockPos, double d, double e, double f, BlockPos blockPos2, float g, float h, double i, double j, double k) {
        ...
        bufferBuilder.vertex(m, o, p).uv(r, t).color(1.0f, 1.0f, 1.0f, (float)l).endVertex();
        bufferBuilder.vertex(m, o, q).uv(r, u).color(1.0f, 1.0f, 1.0f, (float)l).endVertex();
        bufferBuilder.vertex(n, o, q).uv(s, u).color(1.0f, 1.0f, 1.0f, (float)l).endVertex();
        bufferBuilder.vertex(n, o, p).uv(s, t).color(1.0f, 1.0f, 1.0f, (float)l).endVertex();
    }
    ...
    public void postRender(Entity entity, double d, double e, double f, float g, float h) {
        ...
        if (this.entityRenderDispatcher.options.entityShadows && this.shadowRadius > 0.0f && !entity.isInvisible() && this.entityRenderDispatcher.shouldRenderShadow() && (j = (float)((1.0 - (i = this.entityRenderDispatcher.distanceToSqr(entity.x, entity.y, entity.z)) / 256.0) * (double)this.shadowStrength)) > 0.0f) {
    ...
    protected void renderNameTag(T entity, String string, double d, double e, double f, int i) {
        double g = ((Entity)entity).distanceToSqr(this.entityRenderDispatcher.camera.getPosition());
        if (g > (double)(i * i)) {
            return;
        }
        float h = this.entityRenderDispatcher.playerRotY;
        float j = this.entityRenderDispatcher.playerRotX;
        float k = ((Entity)entity).getBbHeight() + 0.5f - (((Entity)entity).isCrouching() ? 0.25f : 0.0f);
        int l = "deadmau5".equals(string) ? -10 : 0;
        GameRenderer.renderNameTagInWorld(this.getFont(), string, (float)d, (float)e + k, (float)f, l, h, j, ((Entity)entity).isDiscrete());
    }
