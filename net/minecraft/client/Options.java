...
public class Options {
    ...
    private final OptionInstance<AmbientOcclusionStatus> ambientOcclusion = new OptionInstance<AmbientOcclusionStatus>("options.ao", Option.noTooltip(), ambientOcclusionStatus -> new TranslatableComponent(ambientOcclusionStatus.getKey()), new OptionInstance.Enum<AmbientOcclusionStatus>(Arrays.asList(AmbientOcclusionStatus.values())), AmbientOcclusionStatus.MAX, ambientOcclusionStatus -> Minecraft.getInstance().levelRenderer.allChanged());
    ...
    }, prioritizeChunkUpdates -> new TranslatableComponent(prioritizeChunkUpdates.getKey()), new OptionInstance.Enum<PrioritizeChunkUpdates>(Arrays.asList(PrioritizeChunkUpdates.values())), PrioritizeChunkUpdates.NONE, prioritizeChunkUpdates -> {});
    ...
    private final OptionInstance<Double> chatHeightUnfocused = new OptionInstance<Double>("options.chat.height.unfocused", Option.noTooltip(), double_ -> Options.pixelValueLabel(this.chatHeightUnfocused().getCaption(), ChatComponent.getHeight(double_)), OptionInstance.UnitDouble.INSTANCE, ChatComponent.defaultUnfocusedPct(), double_ -> Minecraft.getInstance().gui.getChat().rescaleChat());
    private final OptionInstance<Double> chatHeightFocused = new OptionInstance<Double>("options.chat.height.focused", Option.noTooltip(), double_ -> Options.pixelValueLabel(this.chatHeightFocused().getCaption(), ChatComponent.getHeight(double_)), OptionInstance.UnitDouble.INSTANCE, 1.0, double_ -> Minecraft.getInstance().gui.getChat().rescaleChat());
    ...
    private final OptionInstance<Integer> biomeBlendRadius = new OptionInstance<Integer>("options.biomeBlendRadius", Option.noTooltip(), integer -> {
        int i = integer * 2 + 1;
    ...
    private final OptionInstance<Double> mouseWheelSensitivity = new OptionInstance<Double>("options.mouseWheelSensitivity", Option.noTooltip(), double_ -> Options.genericValueLabel(this.mouseWheelSensitivity().getCaption(), new TextComponent(String.format("%.2f", double_))), new OptionInstance.IntRange(-200, 100).xmap(Options::logMouse, Options::unlogMouse), Options.logMouse(0), double_ -> {});
    ...
    private final OptionInstance<Integer> fov = new OptionInstance<Integer>("options.fov", Option.noTooltip(), integer -> {
    ...
    }, new OptionInstance.IntRange(30, 110), 70, integer -> Minecraft.getInstance().levelRenderer.needsUpdate());
    ...
    public Options(Minecraft minecraft2, File file) {
        ...
        if (minecraft2.is64Bit() && Runtime.getRuntime().maxMemory() >= 1000000000L) {
        ...
        this.renderDistance = minecraft2.is64Bit() ? 12 : 8;
        this.simulationDistance = minecraft2.is64Bit() ? 12 : 8;
        this.gamma = 0.5;
    ...
    private void processOptions(FieldAccess fieldAccess) {
        ...
        this.directionalAudio = fieldAccess.process("directionalAudio", this.directionalAudio);
        ...
        this.darkMojangStudiosBackground.set(fieldAccess.process("darkMojangStudiosBackground", this.darkMojangStudiosBackground.get()));
        this.hideLightningFlash.set(fieldAccess.process("hideLightningFlashes", this.hideLightningFlash.get()));
        this.sensitivity = fieldAccess.process("mouseSensitivity", this.sensitivity);
        this.fov.set((int)(fieldAccess.process("fov", (float)(this.fov.get() - 70) / 40.0f) * 40.0f + 70.0f));
        ...
        this.renderDistance = (int)Mth.clamp((double)fieldAccess.process("renderDistance", this.renderDistance), Option.RENDER_DISTANCE.getMinValue(), Option.RENDER_DISTANCE.getMaxValue());
        this.simulationDistance = (int)Mth.clamp((double)fieldAccess.process("simulationDistance", this.simulationDistance), Option.SIMULATION_DISTANCE.getMinValue(), Option.SIMULATION_DISTANCE.getMaxValue());
        ...
        this.ambientOcclusion.set(fieldAccess.process("ao", this.ambientOcclusion.get(), Options::readAmbientOcclusion, ambientOcclusionStatus -> Integer.toString(ambientOcclusionStatus.getId())));
        this.prioritizeChunkUpdates.set(fieldAccess.process("prioritizeChunkUpdates", this.prioritizeChunkUpdates.get(), PrioritizeChunkUpdates::byId, PrioritizeChunkUpdates::getId));
        this.biomeBlendRadius.set(fieldAccess.process("biomeBlendRadius", this.biomeBlendRadius.get()));
        ...
        this.chatHeightFocused.set(fieldAccess.process("chatHeightFocused", this.chatHeightFocused.get()));
        this.chatDelay = fieldAccess.process("chatDelay", this.chatDelay);
        this.chatHeightUnfocused.set(fieldAccess.process("chatHeightUnfocused", this.chatHeightUnfocused.get()));
        ...
        this.mouseWheelSensitivity.set(fieldAccess.process("mouseWheelSensitivity", this.mouseWheelSensitivity.get()));
        this.rawMouseInput.set(fieldAccess.process("rawMouseInput", this.rawMouseInput.get()));
