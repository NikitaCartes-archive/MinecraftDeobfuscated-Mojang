...
public class LayeredRegistryAccess<T> {
    ...
    public LayeredRegistryAccess(List<T> list) {
        this(list, Util.make(() -> {
            Object[] frozens = new RegistryAccess.Frozen[list.size()];
    ...
    private RegistryAccess.Frozen getCompositeAccessForLayers(int i, int j) {
        return new RegistryAccess.ImmutableRegistryAccess(LayeredRegistryAccess.collectRegistries(this.values.subList(i, j).stream())).freeze();
    }
    ...
    public LayeredRegistryAccess<T> replaceFrom(T object, List<RegistryAccess.Frozen> list) {
        ...
        ArrayList<RegistryAccess.Frozen> list2 = new ArrayList<RegistryAccess.Frozen>();
        for (int j = 0; j < i; ++j) {
            list2.add(this.values.get(j));
        }
        list2.addAll(list);
        while (list2.size() < this.values.size()) {
            list2.add(RegistryAccess.EMPTY);
        }
        return new LayeredRegistryAccess<T>(this.keys, list2);
    }
