...
public class Snooper {
    ...
    private void setJvmArgs() {
        int[] is = new int[]{0};
        Util.getVmArguments().forEach(string -> {
            int n = is[0];
            is[0] = n + 1;
            this.setDynamicData("jvm_arg[" + n + "]", string);
        });
    ...
    public Map<String, String> getValues() {
        ...
        synchronized (object) {
            this.prepare();
            for (Map.Entry<String, Object> entry : this.fixedData.entrySet()) {
                map.put(entry.getKey(), entry.getValue().toString());
            }
            for (Map.Entry<String, Object> entry : this.dynamicData.entrySet()) {
                map.put(entry.getKey(), entry.getValue().toString());
            }
        }
        return map;
