...
extends AbstractContainerMenu {
    ...
    public LoomMenu(int i, Inventory inventory, final ContainerLevelAccess containerLevelAccess) {
        ...
        this.resultSlot = this.addSlot(new Slot(this.outputContainer, 0, 143, 58){
            ...
            public ItemStack onTake(Player player, ItemStack itemStack) {
                ...
                containerLevelAccess.execute((level, blockPos) -> level.playSound(null, (BlockPos)blockPos, SoundEvents.UI_LOOM_TAKE_RESULT, SoundSource.BLOCKS, 1.0f, 1.0f));
    ...
    public ItemStack quickMoveStack(Player player, int i) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            } else if (i == this.dyeSlot.index || i == this.bannerSlot.index || i == this.patternSlot.index ? !this.moveItemStackTo(itemStack2, 4, 40, false) : (itemStack2.getItem() instanceof BannerItem ? !this.moveItemStackTo(itemStack2, this.bannerSlot.index, this.bannerSlot.index + 1, false) : (itemStack2.getItem() instanceof DyeItem ? !this.moveItemStackTo(itemStack2, this.dyeSlot.index, this.dyeSlot.index + 1, false) : (itemStack2.getItem() instanceof BannerPatternItem ? !this.moveItemStackTo(itemStack2, this.patternSlot.index, this.patternSlot.index + 1, false) : (i >= 4 && i < 31 ? !this.moveItemStackTo(itemStack2, 31, 40, false) : i >= 31 && i < 40 && !this.moveItemStackTo(itemStack2, 4, 31, false)))))) {
                return ItemStack.EMPTY;
            }
