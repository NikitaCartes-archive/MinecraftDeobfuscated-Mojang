...
extends ItemCombinerMenu {
    ...
    public LegacySmithingMenu(int i, Inventory inventory, ContainerLevelAccess containerLevelAccess) {
        ...
        this.recipes = this.level.getRecipeManager().getAllRecipesFor(RecipeType.SMITHING).stream().filter(smithingRecipe -> smithingRecipe instanceof LegacyUpgradeRecipe).map(smithingRecipe -> (LegacyUpgradeRecipe)smithingRecipe).toList();
    }
    ...
    public void createResult() {
        List<LegacyUpgradeRecipe> list = this.level.getRecipeManager().getRecipesFor(RecipeType.SMITHING, this.inputSlots, this.level).stream().filter(smithingRecipe -> smithingRecipe instanceof LegacyUpgradeRecipe).map(smithingRecipe -> (LegacyUpgradeRecipe)smithingRecipe).toList();
        if (list.isEmpty()) {
