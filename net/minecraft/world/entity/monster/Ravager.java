...
extends Raider {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(4, new NearestAttackableTargetGoal<AbstractVillager>((Mob)this, AbstractVillager.class, true));
    ...
    public void aiStep() {
        ...
        if (this.isImmobile()) {
            ...
            double e = this.getAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getBaseValue();
            this.getAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(Mth.lerp(0.1, e, d));
        }
    ...
    private void stunEffect() {
        if (this.random.nextInt(6) == 0) {
            double d = this.x - (double)this.getBbWidth() * Math.sin(this.yBodyRot * ((float)Math.PI / 180)) + (this.random.nextDouble() * 0.6 - 0.3);
            double e = this.y + (double)this.getBbHeight() - 0.3;
            double f = this.z + (double)this.getBbWidth() * Math.cos(this.yBodyRot * ((float)Math.PI / 180)) + (this.random.nextDouble() * 0.6 - 0.3);
