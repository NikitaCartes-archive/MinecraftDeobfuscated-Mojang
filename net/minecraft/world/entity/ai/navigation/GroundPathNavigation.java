...
extends PathNavigation {
    ...
    protected boolean canMoveDirectly(Vec3 vec3, Vec3 vec32, int i, int j, int k) {
        ...
        if (!this.canWalkOn(l, Mth.floor(vec3.y), m, i += 2, j, k += 2, vec3, d *= g, e *= g)) {
            return false;
        }
        i -= 2;
        k -= 2;
        ...
        while (u * q > 0 || v * r > 0) {
            ...
            if (this.canWalkOn(l, Mth.floor(vec3.y), m, i, j, k, vec3, d, e)) continue;
            return false;
        }
        return true;
    }
    ...
    private boolean canWalkAbove(int i, int j, int k, int l, int m, int n, Vec3 vec3, double d, double e) {
        for (BlockPos blockPos : BlockPos.betweenClosed(new BlockPos(i, j, k), new BlockPos(i + l - 1, j + m - 1, k + n - 1))) {
            ...
            if (f * d + (g = (double)blockPos.getZ() + 0.5 - vec3.z) * e < 0.0 || this.level.getBlockState(blockPos).isPathfindable(this.level, blockPos, PathComputationType.LAND)) continue;
            return false;
        }
        return true;
    }
