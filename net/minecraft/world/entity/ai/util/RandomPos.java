    ...
    private static Vec3 generateRandomPos(PathfinderMob pathfinderMob, int i, int j, int k, @Nullable Vec3 vec3, boolean bl, double d, ToDoubleFunction<BlockPos> toDoubleFunction, boolean bl2, Predicate<BlockPos> predicate, int l, int m, boolean bl3) {
        ...
        for (int n = 0; n < 10; ++n) {
            ...
            if (pathfinderMob.hasRestriction() && i > 1) {
                ...
                o = pathfinderMob.x > (double)blockPos3.getX() ? (o -= random.nextInt(i / 2)) : (o += random.nextInt(i / 2));
                q = pathfinderMob.z > (double)blockPos3.getZ() ? (q -= random.nextInt(i / 2)) : (q += random.nextInt(i / 2));
            }
            blockPos3 = new BlockPos((double)o + pathfinderMob.x, (double)p + pathfinderMob.y, (double)q + pathfinderMob.z);
