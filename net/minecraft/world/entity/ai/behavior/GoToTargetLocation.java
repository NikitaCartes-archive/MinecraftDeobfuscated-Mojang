...
public class GoToTargetLocation {
    ...
    public static <E extends Mob> OneShot<E> create(MemoryModuleType<BlockPos> memoryModuleType, int i, float f) {
        return BehaviorBuilder.create(instance -> instance.group(instance.present(memoryModuleType), instance.absent(MemoryModuleType.ATTACK_TARGET), instance.absent(MemoryModuleType.WALK_TARGET), instance.registered(MemoryModuleType.LOOK_TARGET)).apply(instance, (memoryAccessor, memoryAccessor2, memoryAccessor3, memoryAccessor4) -> (serverLevel, mob, l) -> {
            BlockPos blockPos = (BlockPos)instance.get(memoryAccessor);
            boolean bl = blockPos.closerThan(mob.blockPosition(), i);
            if (!bl) {
                BehaviorUtils.setWalkAndLookTargetMemories(mob, GoToTargetLocation.getNearbyPos(mob, blockPos), f, i);
            }
            return true;
        }));
    }
