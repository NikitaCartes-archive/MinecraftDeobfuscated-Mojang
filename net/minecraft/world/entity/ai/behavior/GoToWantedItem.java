...
public class GoToWantedItem {
    public static BehaviorControl<LivingEntity> create(float f, boolean bl, int i) {
        return GoToWantedItem.create(livingEntity -> true, f, bl, i);
    }
    ...
    public static <E extends LivingEntity> BehaviorControl<E> create(Predicate<E> predicate, float f, boolean bl, int i) {
        return BehaviorBuilder.create(instance -> {
            ...
            return instance.group(instance.registered(MemoryModuleType.LOOK_TARGET), behaviorBuilder, instance.present(MemoryModuleType.NEAREST_VISIBLE_WANTED_ITEM), instance.registered(MemoryModuleType.ITEM_PICKUP_COOLDOWN_TICKS)).apply(instance, (memoryAccessor, memoryAccessor2, memoryAccessor3, memoryAccessor4) -> (serverLevel, livingEntity, l) -> {
                ItemEntity itemEntity = (ItemEntity)instance.get(memoryAccessor3);
                if (instance.tryGet(memoryAccessor4).isEmpty() && predicate.test(livingEntity) && itemEntity.closerThan(livingEntity, i) && livingEntity.level.getWorldBorder().isWithinBounds(itemEntity.blockPosition())) {
