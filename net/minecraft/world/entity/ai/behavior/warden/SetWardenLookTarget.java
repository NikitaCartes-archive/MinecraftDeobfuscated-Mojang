...
extends Behavior<Warden> {
    ...
    protected void start(ServerLevel serverLevel, Warden warden, long l) {
        BlockPos blockPos = warden.getBrain().getMemory(MemoryModuleType.ROAR_TARGET).map(Entity::blockPosition).or(() -> warden.getBrain().getMemory(MemoryModuleType.DISTURBANCE_LOCATION)).get();
