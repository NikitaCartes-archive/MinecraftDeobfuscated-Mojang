...
extends SectionStorage<PoiSection> {
    ...
    public PoiManager(Path path, DataFixer dataFixer, boolean bl, RegistryAccess registryAccess, LevelHeightAccessor levelHeightAccessor) {
        super(path, PoiSection::codec, PoiSection::new, dataFixer, DataFixTypes.POI_CHUNK, bl, registryAccess, levelHeightAccessor);
    ...
    public Stream<PoiRecord> getInSquare(Predicate<Holder<PoiType>> predicate, BlockPos blockPos, int i, Occupancy occupancy) {
        ...
        return ChunkPos.rangeClosed(new ChunkPos(blockPos), j).flatMap(chunkPos -> this.getInChunk(predicate, (ChunkPos)chunkPos, occupancy)).filter(poiRecord -> {
    ...
    public Stream<Pair<Holder<PoiType>, BlockPos>> findAllWithType(Predicate<Holder<PoiType>> predicate, Predicate<BlockPos> predicate2, BlockPos blockPos, int i, Occupancy occupancy) {
        return this.getInRange(predicate, blockPos, i, occupancy).filter(poiRecord -> predicate2.test(poiRecord.getPos())).map(poiRecord -> Pair.of(poiRecord.getPoiType(), poiRecord.getPos()));
    }
    ...
    public Stream<Pair<Holder<PoiType>, BlockPos>> findAllClosestFirstWithType(Predicate<Holder<PoiType>> predicate, Predicate<BlockPos> predicate2, BlockPos blockPos, int i, Occupancy occupancy) {
        return this.findAllWithType(predicate, predicate2, blockPos, i, occupancy).sorted(Comparator.comparingDouble(pair -> ((BlockPos)pair.getSecond()).distSqr(blockPos)));
    }
    ...
    public Optional<Pair<Holder<PoiType>, BlockPos>> findClosestWithType(Predicate<Holder<PoiType>> predicate, BlockPos blockPos, int i, Occupancy occupancy) {
        return this.getInRange(predicate, blockPos, i, occupancy).min(Comparator.comparingDouble(poiRecord -> poiRecord.getPos().distSqr(blockPos))).map(poiRecord -> Pair.of(poiRecord.getPoiType(), poiRecord.getPos()));
    }
    ...
    public Optional<BlockPos> findClosest(Predicate<Holder<PoiType>> predicate, Predicate<BlockPos> predicate2, BlockPos blockPos, int i, Occupancy occupancy) {
        return this.getInRange(predicate, blockPos, i, occupancy).map(PoiRecord::getPos).filter(predicate2).min(Comparator.comparingDouble(blockPos2 -> blockPos2.distSqr(blockPos)));
    }
    ...
    public Optional<BlockPos> take(Predicate<Holder<PoiType>> predicate, BiPredicate<Holder<PoiType>, BlockPos> biPredicate, BlockPos blockPos, int i) {
        return this.getInRange(predicate, blockPos, i, Occupancy.HAS_SPACE).filter(poiRecord -> biPredicate.test(poiRecord.getPoiType(), poiRecord.getPos())).findFirst().map(poiRecord -> {
