...
extends AbstractArrow {
    ...
    public void setEffectsFromItem(ItemStack itemStack) {
        if (itemStack.getItem() == Items.TIPPED_ARROW) {
            ...
            List<MobEffectInstance> collection = PotionUtils.getCustomEffects(itemStack);
            if (!collection.isEmpty()) {
                for (MobEffectInstance mobEffectInstance : collection) {
                    this.effects.add(new MobEffectInstance(mobEffectInstance));
                }
            }
            if ((i = Arrow.getCustomColor(itemStack)) == -1) {
                this.updateColor();
            } else {
    ...
    private void makeParticle(int i) {
        ...
        for (int k = 0; k < i; ++k) {
            this.level.addParticle(ParticleTypes.ENTITY_EFFECT, this.x + (this.random.nextDouble() - 0.5) * (double)this.getBbWidth(), this.y + this.random.nextDouble() * (double)this.getBbHeight(), this.z + (this.random.nextDouble() - 0.5) * (double)this.getBbWidth(), d, e, f);
        }
    }
