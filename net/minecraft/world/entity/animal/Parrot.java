...
implements FlyingAnimal {
    ...
    private static boolean imitateNearbyMobs(Level level, Entity entity) {
        ...
        if (!list.isEmpty() && !(mob = list.get(level.random.nextInt(list.size()))).isSilent()) {
            ...
            level.playSound(null, entity.x, entity.y, entity.z, soundEvent, entity.getSoundSource(), 0.7f, Parrot.getPitch(level.random));
            return true;
        }
        return false;
    }
    ...
    public static void playAmbientSound(Level level, Entity entity) {
        if (!entity.isSilent() && !Parrot.imitateNearbyMobs(level, entity) && level.random.nextInt(200) == 0) {
            level.playSound(null, entity.x, entity.y, entity.z, Parrot.getAmbient(level.random), entity.getSoundSource(), 1.0f, Parrot.getPitch(level.random));
        }
    }
