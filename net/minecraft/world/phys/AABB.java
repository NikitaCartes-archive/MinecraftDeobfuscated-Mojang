...
public class AABB {
    ...
    public Optional<Vec3> clip(Vec3 vec3, Vec3 vec32) {
        ...
        Direction direction = AABB.getDirection(this, vec3, ds, null, d, e, f);
        if (direction == null) {
    ...
    public static BlockHitResult clip(Iterable<AABB> iterable, Vec3 vec3, Vec3 vec32, BlockPos blockPos) {
        ...
        double f = vec32.z - vec3.z;
        for (AABB aABB : iterable) {
            direction = AABB.getDirection(aABB.move(blockPos), vec3, ds, direction, d, e, f);
        }
        if (direction == null) {
            return null;
        }
    ...
    private static Direction getDirection(AABB aABB, Vec3 vec3, double[] ds, @Nullable Direction direction, double d, double e, double f) {
        if (d > 1.0E-7) {
            direction = AABB.clipPoint(ds, direction, d, e, f, aABB.minX, aABB.minY, aABB.maxY, aABB.minZ, aABB.maxZ, Direction.WEST, vec3.x, vec3.y, vec3.z);
        } else if (d < -1.0E-7) {
            direction = AABB.clipPoint(ds, direction, d, e, f, aABB.maxX, aABB.minY, aABB.maxY, aABB.minZ, aABB.maxZ, Direction.EAST, vec3.x, vec3.y, vec3.z);
        }
        if (e > 1.0E-7) {
            direction = AABB.clipPoint(ds, direction, e, f, d, aABB.minY, aABB.minZ, aABB.maxZ, aABB.minX, aABB.maxX, Direction.DOWN, vec3.y, vec3.z, vec3.x);
        } else if (e < -1.0E-7) {
            direction = AABB.clipPoint(ds, direction, e, f, d, aABB.maxY, aABB.minZ, aABB.maxZ, aABB.minX, aABB.maxX, Direction.UP, vec3.y, vec3.z, vec3.x);
        }
        if (f > 1.0E-7) {
            direction = AABB.clipPoint(ds, direction, f, d, e, aABB.minZ, aABB.minX, aABB.maxX, aABB.minY, aABB.maxY, Direction.NORTH, vec3.z, vec3.x, vec3.y);
        } else if (f < -1.0E-7) {
            direction = AABB.clipPoint(ds, direction, f, d, e, aABB.maxZ, aABB.minX, aABB.maxX, aABB.minY, aABB.maxY, Direction.SOUTH, vec3.z, vec3.x, vec3.y);
        }
        return direction;
    }

    @Nullable
    private static Direction clipPoint(double[] ds, @Nullable Direction direction, double d, double e, double f, double g, double h, double i, double j, double k, Direction direction2, double l, double m, double n) {
        double o = (g - l) / d;
        double p = m + o * e;
        double q = n + o * f;
