...
public abstract class ChunkGenerator {
    ...
    private void generateStrongholds() {
        ...
        for (Biome biome : this.biomeSource.possibleBiomes()) {
            if (!biome.isValidStart(StructureFeature.STRONGHOLD)) continue;
            list.add(biome);
        }
        ...
        for (int n = 0; n < j; ++n) {
            ...
            BlockPos blockPos = this.biomeSource.findBiomeHorizontal((o << 4) + 8, 0, ((p = (int)Math.round(Math.sin(d) * e)) << 4) + 8, 112, list, random);
            if (blockPos != null) {
                o = blockPos.getX() >> 4;
                p = blockPos.getZ() >> 4;
            }
        ...
        for (int m = j - 8; m <= j + 8; ++m) {
            for (int n = k - 8; n <= k + 8; ++n) {
                ...
                while (listIterator.hasNext()) {
                    ...
                    ConfiguredWorldCarver<?> configuredWorldCarver = listIterator.next();
                    worldgenRandom.setLargeFeatureSeed(l + (long)o, m, n);
                    if (!configuredWorldCarver.isStartChunk(worldgenRandom, m, n)) continue;
    ...
    public void applyBiomeDecoration(WorldGenRegion worldGenRegion, StructureFeatureManager structureFeatureManager) {
        ...
        for (GenerationStep.Decoration decoration : GenerationStep.Decoration.values()) {
            try {
                ...
                crashReport.addCategory("Generation").setDetail("CenterX", i).setDetail("CenterZ", j).setDetail("Step", decoration).setDetail("Seed", m).setDetail("Biome", Registry.BIOME.getKey(biome));
    ...
    private void createStructure(ConfiguredStructureFeature<?, ?> configuredStructureFeature, StructureFeatureManager structureFeatureManager, ChunkAccess chunkAccess, StructureManager structureManager, long l, ChunkPos chunkPos, Biome biome) {
        ...
        StructureStart<?> structureStart2 = configuredStructureFeature.generate(this, this.biomeSource, structureManager, l, chunkPos, biome, i, this.settings.getConfig((StructureFeature<?>)configuredStructureFeature.feature));
