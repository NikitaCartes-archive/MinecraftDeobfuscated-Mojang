...
public abstract class ChunkGenerator {
        ...
        for (int m = j - 8; m <= j + 8; ++m) {
            for (int n = k - 8; n <= k + 8; ++n) {
                ...
                while (listIterator.hasNext()) {
                    ...
                    configuredWorldCarver.carve(chunkAccess, blockPos -> this.getCarvingOrDecorationBiome(biomeManager, (BlockPos)blockPos), worldgenRandom, this.getSeaLevel(), m, n, j, k, bitSet);
                }
            }
        }
    }
    ...
    public void createStructures(StructureFeatureManager structureFeatureManager, BiomeManager biomeManager, ChunkAccess chunkAccess, ChunkGenerator chunkGenerator, StructureManager structureManager, long l) {
        for (StructureFeature structureFeature : Feature.STRUCTURES_REGISTRY.values()) {
            if (!chunkGenerator.canGenerateStructure(structureFeature)) continue;
