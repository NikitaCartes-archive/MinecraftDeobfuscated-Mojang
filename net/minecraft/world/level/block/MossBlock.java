...
implements BonemealableBlock {
    ...
    public void performBonemeal(ServerLevel serverLevel, Random random, BlockPos blockPos, BlockState blockState) {
        MossBlock.place(serverLevel, random, blockPos.above());
    }
    ...
    public static boolean place(WorldGenLevel worldGenLevel, Random random, BlockPos blockPos) {
        ...
        for (int l = -j; l <= j; ++l) {
            for (int m = -k; m <= k; ++m) {
                ...
                i += MossBlock.placeFeature(worldGenLevel, random, blockPos2);
            }
        }
        return i > 0;
    }
    ...
    private static int placeFeature(WorldGenLevel worldGenLevel, Random random, BlockPos blockPos) {
        ...
        if (worldGenLevel.isEmptyBlock(blockPos) && blockState.isFaceSturdy(worldGenLevel, blockPos2, Direction.UP)) {
            ...
            MossBlock.createMossPatch(worldGenLevel, random, blockPos.below());
            if (random.nextFloat() < 0.8f && (blockState2 = MossBlock.getVegetationBlockState(random)).canSurvive(worldGenLevel, blockPos)) {
