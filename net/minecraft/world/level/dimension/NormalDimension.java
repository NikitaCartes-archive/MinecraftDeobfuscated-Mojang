...
extends Dimension {
    ...
    public ChunkGenerator<? extends ChunkGeneratorSettings> createRandomLevelGenerator() {
        ...
        if (levelType == LevelType.FLAT) {
            FlatLevelGeneratorSettings flatLevelGeneratorSettings = FlatLevelGeneratorSettings.fromObject(new Dynamic<CompoundTag>(NbtOps.INSTANCE, this.level.getLevelData().getGeneratorOptions()));
            FixedBiomeSourceSettings fixedBiomeSourceSettings = biomeSourceType.createSettings(this.level.getLevelData()).setBiome(flatLevelGeneratorSettings.getBiome());
        ...
        if (levelType == LevelType.BUFFET) {
            ...
            if (jsonObject2 != null && jsonObject2.has("type") && jsonObject2.has("options")) {
                ...
                if (jsonObject3.has("biomes")) {
                    ...
                    for (int i = 0; i < jsonArray.size(); ++i) {
                        biomes[i] = Registry.BIOME.getOptional(new ResourceLocation(jsonArray.get(i).getAsString())).orElse(Biomes.OCEAN);
                    }
                }
                ...
                if (BiomeSourceType.CHECKERBOARD == biomeSourceType4) {
                    ...
                    CheckerboardBiomeSourceSettings checkerboardBiomeSourceSettings = biomeSourceType3.createSettings(this.level.getLevelData()).setAllowedBiomes(biomes).setSize(j);
            ...
            if (jsonObject4 != null && jsonObject4.has("type")) {
                ChunkGeneratorType<?, ?> chunkGeneratorType6 = Registry.CHUNK_GENERATOR_TYPE.get(new ResourceLocation(jsonObject4.getAsJsonPrimitive("type").getAsString()));
        ...
        OverworldBiomeSourceSettings overworldBiomeSourceSettings2 = biomeSourceType2.createSettings(this.level.getLevelData()).setGeneratorSettings(overworldGeneratorSettings2);
