...
extends BiomeSource {
    ...
    public Biome getNoiseBiome(int i, int j, int k) {
        ...
        Biome.ClimateParameters climateParameters = new Biome.ClimateParameters((float)((this.temperatureNoise.getValue(f, 0.0, g) + this.temperatureNoise.getValue(h, 0.0, l)) * 0.5), (float)((this.humidityNoise.getValue(f, 0.0, g) + this.humidityNoise.getValue(h, 0.0, l)) * 0.5), (float)((this.altitudeNoise.getValue(f, 0.0, g) + this.altitudeNoise.getValue(h, 0.0, l)) * 0.5), (float)((this.weirdnessNoise.getValue(f, 0.0, g) + this.weirdnessNoise.getValue(h, 0.0, l)) * 0.5), 1.0f);
        return this.possibleBiomes.stream().min(Comparator.comparing(biome -> Float.valueOf(biome.getFitness(climateParameters)))).orElse(Biomes.THE_END);
    }
}
