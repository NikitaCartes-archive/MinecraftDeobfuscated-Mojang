...
extends Biome {
    public WarmOceanBiome() {
        super(new Biome.BiomeBuilder().surfaceBuilder(SurfaceBuilder.DEFAULT, SurfaceBuilder.CONFIG_FULL_SAND).precipitation(Biome.Precipitation.RAIN).biomeCategory(Biome.BiomeCategory.OCEAN).depth(-1.0f).scale(0.1f).temperature(0.5f).downfall(0.5f).waterColor(4445678).waterFogColor(270131).parent(null));
        ...
        this.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, WarmOceanBiome.makeComposite(Feature.SIMPLE_RANDOM_SELECTOR, new SimpleRandomFeatureConfig(new Feature[]{Feature.CORAL_TREE, Feature.CORAL_CLAW, Feature.CORAL_MUSHROOM}, new FeatureConfiguration[]{FeatureConfiguration.NONE, FeatureConfiguration.NONE, FeatureConfiguration.NONE}), FeatureDecorator.TOP_SOLID_HEIGHTMAP_NOISE_BIASED, new DecoratorNoiseCountFactor(20, 400.0, 0.0, Heightmap.Types.OCEAN_FLOOR_WG)));
        BiomeDefaultFeatures.addWarmSeagrass(this);
        this.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, WarmOceanBiome.makeComposite(Feature.SEA_PICKLE, new CountFeatureConfiguration(20), FeatureDecorator.CHANCE_TOP_SOLID_HEIGHTMAP, new DecoratorChance(16)));
