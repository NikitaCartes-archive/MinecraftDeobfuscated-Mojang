...
public class NetherFossilPieces {
    ...
    public static void addPieces(StructureManager structureManager, List<StructurePiece> list, Random random, BlockPos blockPos) {
        ...
        list.add(new NetherFossilPiece(structureManager, FOSSILS[random.nextInt(FOSSILS.length)], blockPos, rotation));
    }
    ...
    extends TemplateStructurePiece {
        ...
        public boolean postProcess(LevelAccessor levelAccessor, ChunkGenerator<?> chunkGenerator, Random random, BoundingBox boundingBox, ChunkPos chunkPos) {
            while (levelAccessor.getBlockState(this.templatePosition.above()).getBlock() != Blocks.BEDROCK && this.templatePosition.getY() < 128) {
                ...
                if (levelAccessor.getBlockState(this.templatePosition).isAir() && (blockState.getBlock() == Blocks.SOUL_SAND || blockState.isFaceSturdy(levelAccessor, blockPos, Direction.UP))) break;
