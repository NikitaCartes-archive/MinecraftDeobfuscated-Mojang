...
public final class StructureStart {
    ...
    public BoundingBox getBoundingBox() {
        ...
        if (boundingBox == null) {
            this.cachedBoundingBox = boundingBox = this.feature.adjustBoundingBox(this.pieceContainer.calculateBoundingBox());
        }
        return boundingBox;
    }
    ...
    public void placeInChunk(WorldGenLevel worldGenLevel, StructureFeatureManager structureFeatureManager, ChunkGenerator chunkGenerator, Random random, BoundingBox boundingBox, ChunkPos chunkPos) {
        ...
        ((StructureFeature)this.feature.feature).getPostPlacementProcessor().afterPlace(worldGenLevel, structureFeatureManager, chunkGenerator, random, boundingBox, chunkPos, this.pieceContainer);
    }
    ...
    public CompoundTag createTag(StructurePieceSerializationContext structurePieceSerializationContext, ChunkPos chunkPos) {
        ...
        compoundTag.putString("id", structurePieceSerializationContext.registryAccess().registryOrThrow(Registry.CONFIGURED_STRUCTURE_FEATURE_REGISTRY).getKey(this.feature).toString());
