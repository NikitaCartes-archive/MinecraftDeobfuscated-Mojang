...
public final class NoiseGeneratorSettings {
    ...
    private NoiseGeneratorSettings(StructureSettings structureSettings, NoiseSettings noiseSettings, BlockState blockState, BlockState blockState2, int i, int j, int k, boolean bl, Optional<Preset> optional) {
    ...
    public static class Preset {
        ...
        public static final Codec<Preset> CODEC = ResourceLocation.CODEC.flatXmap(resourceLocation -> Optional.ofNullable(BY_NAME.get(resourceLocation)).map(DataResult::success).orElseGet(() -> DataResult.error("Unknown preset: " + resourceLocation)), preset -> DataResult.success(preset.name)).stable();
        ...
        public Preset(String string, Function<Preset, NoiseGeneratorSettings> function) {
            this.name = new ResourceLocation(string);
            this.description = new TranslatableComponent("generator.noise." + string);
            this.settings = function.apply(this);
