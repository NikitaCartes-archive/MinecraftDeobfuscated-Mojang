...
extends Feature<DoublePlantConfiguration> {
    public DoublePlantFeature(Function<Dynamic<?>, ? extends DoublePlantConfiguration> function) {
        super(function);
    }

    @Override
    public boolean place(LevelAccessor levelAccessor, ChunkGenerator<? extends ChunkGeneratorSettings> chunkGenerator, Random random, BlockPos blockPos, DoublePlantConfiguration doublePlantConfiguration) {
        boolean bl = false;
        for (int i = 0; i < 64; ++i) {
            ...
            if (!levelAccessor.isEmptyBlock(blockPos2) || blockPos2.getY() >= 254 || !doublePlantConfiguration.state.canSurvive(levelAccessor, blockPos2)) continue;
