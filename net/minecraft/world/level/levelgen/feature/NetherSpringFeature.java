...
extends Feature<HellSpringConfiguration> {
    ...
    @Override
    public boolean place(LevelAccessor levelAccessor, ChunkGenerator<? extends ChunkGeneratorSettings> chunkGenerator, Random random, BlockPos blockPos, HellSpringConfiguration hellSpringConfiguration) {
        if (levelAccessor.getBlockState(blockPos.above()) != NETHERRACK) {
            return false;
        }
        if (!levelAccessor.getBlockState(blockPos).isAir() && levelAccessor.getBlockState(blockPos) != NETHERRACK) {
            return false;
        }
        int i = 0;
