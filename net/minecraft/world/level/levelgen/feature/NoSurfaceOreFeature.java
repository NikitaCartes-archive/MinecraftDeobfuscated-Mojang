...
extends Feature<OreConfiguration> {
    ...
    @Override
    public boolean place(LevelAccessor levelAccessor, ChunkGenerator<? extends ChunkGeneratorSettings> chunkGenerator, Random random, BlockPos blockPos, OreConfiguration oreConfiguration) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (!oreConfiguration.target.getPredicate().test(levelAccessor.getBlockState(mutableBlockPos)) || this.isFacingAir(levelAccessor, mutableBlockPos)) continue;
