...
extends AbstractTreeFeature<T> {
    ...
    protected void placeDoubleTrunkLeaves(LevelSimulatedRW levelSimulatedRW, Random random, BlockPos blockPos, int i, Set<BlockPos> set, BoundingBox boundingBox, TreeConfiguration treeConfiguration) {
        int j = i * i;
        for (int k = -i; k <= i + 1; ++k) {
            for (int l = -i; l <= i + 1; ++l) {
                int n;
    ...
    protected void placeSingleTrunkLeaves(LevelSimulatedRW levelSimulatedRW, Random random, BlockPos blockPos, int i, Set<BlockPos> set, BoundingBox boundingBox, TreeConfiguration treeConfiguration) {
        int j = i * i;
        for (int k = -i; k <= i; ++k) {
            for (int l = -i; l <= i; ++l) {
    ...
    protected void placeTrunk(LevelSimulatedRW levelSimulatedRW, Random random, BlockPos blockPos, int i, Set<BlockPos> set, BoundingBox boundingBox, MegaTreeConfiguration megaTreeConfiguration) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (MegaTreeFeature.isFree(levelSimulatedRW, mutableBlockPos)) {
                MegaTreeFeature.placeLog(levelSimulatedRW, random, mutableBlockPos, set, boundingBox, megaTreeConfiguration);
            }
            if (j >= i - 1) continue;
