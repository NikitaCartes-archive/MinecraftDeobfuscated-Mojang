...
extends Feature<BushConfiguration> {
    ...
    @Override
    public boolean place(LevelAccessor levelAccessor, ChunkGenerator<? extends ChunkGeneratorSettings> chunkGenerator, Random random, BlockPos blockPos, BushConfiguration bushConfiguration) {
        int i = 0;
        ...
        for (int j = 0; j < 64; ++j) {
            ...
            if (!levelAccessor.isEmptyBlock(blockPos2) || levelAccessor.getDimension().isHasCeiling() && blockPos2.getY() >= 255 || !blockState.canSurvive(levelAccessor, blockPos2)) continue;
