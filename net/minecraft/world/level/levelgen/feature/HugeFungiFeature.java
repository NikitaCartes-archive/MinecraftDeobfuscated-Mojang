...
extends Feature<HugeFungiConfiguration> {
    public HugeFungiFeature(Function<Dynamic<?>, ? extends HugeFungiConfiguration> function) {
        super(function);
    }

    @Override
    public boolean place(LevelAccessor levelAccessor, ChunkGenerator<? extends ChunkGeneratorSettings> chunkGenerator, Random random, BlockPos blockPos, HugeFungiConfiguration hugeFungiConfiguration) {
        BlockPos.MutableBlockPos mutableBlockPos = HugeFungiFeature.findOnNyliumPosition(levelAccessor, blockPos);
    ...
    private void placeStem(LevelAccessor levelAccessor, Random random, HugeFungiConfiguration hugeFungiConfiguration, BlockPos.MutableBlockPos mutableBlockPos, int i, boolean bl) {
        ...
        for (int k = -j; k <= j; ++k) {
            for (int l = -j; l <= j; ++l) {
                ...
                for (int m = 0; m < i; ++m) {
                    ...
                    if (levelAccessor.getBlockState(mutableBlockPos2).isSolidRender(levelAccessor, mutableBlockPos2)) continue;
    ...
    private void placeHat(LevelAccessor levelAccessor, Random random, HugeFungiConfiguration hugeFungiConfiguration, BlockPos.MutableBlockPos mutableBlockPos, int i, boolean bl) {
        int k;
        BlockPos.MutableBlockPos mutableBlockPos2 = new BlockPos.MutableBlockPos();
        ...
        for (int l = k = i - j; l <= i; ++l) {
            ...
            for (int n2 = -m; n2 <= m; ++n2) {
                for (int o = -m; o <= m; ++o) {
                    ...
                    boolean bl6 = bl3 && bl4;
                    boolean bl7 = l < k + 3;
                    mutableBlockPos2.set(mutableBlockPos).move(n2, l, o);
                    if (levelAccessor.getBlockState(mutableBlockPos2).isSolidRender(levelAccessor, mutableBlockPos2)) continue;
                    if (bl7) {
                        if (bl5) continue;
    ...
    private void placeHatBlock(LevelAccessor levelAccessor, Random random, HugeFungiConfiguration hugeFungiConfiguration, BlockPos.MutableBlockPos mutableBlockPos, float f, float g, float h) {
    ...
    private static BlockPos.MutableBlockPos findOnNyliumPosition(LevelAccessor levelAccessor, BlockPos blockPos) {
        ...
        for (int i = blockPos.getY(); i >= 1; --i) {
            ...
            Block block = levelAccessor.getBlockState((BlockPos)mutableBlockPos.below()).getBlock();
            if (!block.is(BlockTags.NYLIUM)) continue;
