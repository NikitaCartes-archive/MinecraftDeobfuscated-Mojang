...
public class ConfiguredStructureFeature<FC extends FeatureConfiguration, F extends StructureFeature<FC>> {
    ...
    public ConfiguredStructureFeature(F structureFeature, FC featureConfiguration, HolderSet<Biome> holderSet, boolean bl, Map<MobCategory, StructureSpawnOverride> map) {
    ...
    public StructureStart generate(RegistryAccess registryAccess, ChunkGenerator chunkGenerator, BiomeSource biomeSource, StructureManager structureManager, long l, ChunkPos chunkPos, int i, LevelHeightAccessor levelHeightAccessor, Predicate<Holder<Biome>> predicate) {
        Optional optional = ((StructureFeature)this.feature).pieceGeneratorSupplier().createGenerator(new PieceGeneratorSupplier.Context<FC>(chunkGenerator, biomeSource, l, chunkPos, this.config, levelHeightAccessor, predicate, structureManager, registryAccess));
        if (optional.isPresent()) {
            StructurePiecesBuilder structurePiecesBuilder = new StructurePiecesBuilder();
            WorldgenRandom worldgenRandom = new WorldgenRandom(new LegacyRandomSource(0L));
            worldgenRandom.setLargeFeatureSeed(l, chunkPos.x, chunkPos.z);
            optional.get().generatePieces(structurePiecesBuilder, new PieceGenerator.Context<FC>(this.config, chunkGenerator, structureManager, chunkPos, levelHeightAccessor, worldgenRandom, l));
