...
public abstract class RandomScatteredFeature<C extends FeatureConfiguration>
extends StructureFeature<C> {
    ...
    public boolean isFeatureChunk(ChunkGenerator<?> chunkGenerator, Random random, int i, int j) {
        ...
        return i == chunkPos.x && j == chunkPos.z && chunkGenerator.isBiomeValidStartForStructure(biome = chunkGenerator.getBiomeSource().getBiome(new BlockPos(i * 16 + 9, 0, j * 16 + 9)), this);
    }

    protected int getSpacing(ChunkGenerator<?> chunkGenerator) {
        return ((ChunkGeneratorSettings)chunkGenerator.getSettings()).getTemplesSpacing();
    }

    protected int getSeparation(ChunkGenerator<?> chunkGenerator) {
        return ((ChunkGeneratorSettings)chunkGenerator.getSettings()).getTemplesSeparation();
    }
