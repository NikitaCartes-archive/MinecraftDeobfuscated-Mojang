...
implements FeatureConfiguration {
    ...
    public static <T> ReplaceSpheroidConfiguration deserialize(Dynamic<T> dynamic) {
        BlockState blockState = dynamic.get("target").map(BlockState::deserialize).orElse(Blocks.AIR.defaultBlockState());
        BlockState blockState2 = dynamic.get("state").map(BlockState::deserialize).orElse(Blocks.AIR.defaultBlockState());
