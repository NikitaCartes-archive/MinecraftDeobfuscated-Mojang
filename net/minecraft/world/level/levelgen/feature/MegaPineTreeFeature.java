...
extends MegaTreeFeature<NoneFeatureConfiguration> {
    ...
    public boolean doPlace(Set<BlockPos> set, LevelSimulatedRW levelSimulatedRW, Random random, BlockPos blockPos, BoundingBox boundingBox) {
        ...
        for (int j = 0; j < i; ++j) {
            if (MegaPineTreeFeature.isAirOrLeaves(levelSimulatedRW, blockPos.above(j))) {
                this.setBlock(set, levelSimulatedRW, blockPos.above(j), this.trunk, boundingBox);
            }
            if (j >= i - 1) continue;
            if (MegaPineTreeFeature.isAirOrLeaves(levelSimulatedRW, blockPos.offset(1, j, 0))) {
                this.setBlock(set, levelSimulatedRW, blockPos.offset(1, j, 0), this.trunk, boundingBox);
            }
            if (MegaPineTreeFeature.isAirOrLeaves(levelSimulatedRW, blockPos.offset(1, j, 1))) {
                this.setBlock(set, levelSimulatedRW, blockPos.offset(1, j, 1), this.trunk, boundingBox);
            }
            if (!MegaPineTreeFeature.isAirOrLeaves(levelSimulatedRW, blockPos.offset(0, j, 1))) continue;
    ...
    private void placePodzolAt(LevelSimulatedRW levelSimulatedRW, BlockPos blockPos) {
        for (int i = 2; i >= -3; --i) {
            ...
            if (MegaPineTreeFeature.isGrassOrDirt(levelSimulatedRW, blockPos2)) {
                this.setBlock(levelSimulatedRW, blockPos2, PODZOL);
                break;
            }
            if (!MegaPineTreeFeature.isAir(levelSimulatedRW, blockPos2) && i < 0) break;
        }
    }
}
