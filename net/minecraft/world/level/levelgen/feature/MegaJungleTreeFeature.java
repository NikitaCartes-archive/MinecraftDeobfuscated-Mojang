...
extends MegaTreeFeature<MegaTreeConfiguration> {
    ...
    public boolean doPlace(LevelSimulatedRW levelSimulatedRW, Random random, BlockPos blockPos, Set<BlockPos> set, Set<BlockPos> set2, BoundingBox boundingBox, MegaTreeConfiguration megaTreeConfiguration) {
        ...
        for (int j = blockPos.getY() + i - 2 - random.nextInt(4); j > blockPos.getY() + i / 2; j -= 2 + random.nextInt(4)) {
            ...
            for (m = 0; m < 5; ++m) {
                ...
                MegaJungleTreeFeature.placeLog(levelSimulatedRW, random, blockPos2, set, boundingBox, megaTreeConfiguration);
            }
    ...
    private void createCrown(LevelSimulatedRW levelSimulatedRW, Random random, BlockPos blockPos, int i, Set<BlockPos> set, BoundingBox boundingBox, TreeConfiguration treeConfiguration) {
        int j = 2;
        for (int k = -2; k <= 0; ++k) {
