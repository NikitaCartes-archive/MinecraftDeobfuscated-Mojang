...
extends Feature<ReplaceSphereConfiguration> {
    ...
    public boolean place(WorldGenLevel worldGenLevel, ChunkGenerator chunkGenerator, Random random, BlockPos blockPos, ReplaceSphereConfiguration replaceSphereConfiguration) {
        ...
        BlockPos blockPos2 = ReplaceBlobsFeature.findTarget(worldGenLevel, blockPos.mutable().clamp(Direction.Axis.Y, worldGenLevel.getMinBuildHeight() + 1, worldGenLevel.getMaxBuildHeight() - 1), block);
        if (blockPos2 == null) {
            return false;
        }
        ...
        for (BlockPos blockPos3 : BlockPos.withinManhattan(blockPos2, i, j, k)) {
            if (blockPos3.distManhattan(blockPos2) > l) break;
