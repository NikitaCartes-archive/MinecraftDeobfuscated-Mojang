...
extends StructureFeature<NoneFeatureConfiguration> {
    ...
    private static Optional<PieceGenerator<NoneFeatureConfiguration>> pieceGeneratorSupplier(PieceGeneratorSupplier.Context<NoneFeatureConfiguration> context2) {
        ...
        int k = context2.chunkPos().getBlockX(7);
        int l = context2.chunkPos().getBlockZ(7);
        int[] is = context2.getCornerHeights(k, i, l, j);
        int m = Math.min(Math.min(is[0], is[1]), Math.min(is[2], is[3]));
        if (m < 60) {
            return Optional.empty();
        }
        if (!context2.validBiome().test(context2.chunkGenerator().getNoiseBiome(QuartPos.fromBlock(k), QuartPos.fromBlock(is[0]), QuartPos.fromBlock(l)))) {
            return Optional.empty();
        }
        BlockPos blockPos = new BlockPos(context2.chunkPos().getMiddleBlockX(), m + 1, context2.chunkPos().getMiddleBlockZ());
        return Optional.of((structurePiecesBuilder, context) -> {
            ...
            WoodlandMansionPieces.generateMansion(context.structureManager(), blockPos, rotation, list, worldgenRandom);
