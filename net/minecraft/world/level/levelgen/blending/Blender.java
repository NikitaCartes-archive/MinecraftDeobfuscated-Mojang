...
public class Blender {
    ...
    public static Blender of(@Nullable WorldGenRegion worldGenRegion) {
        ...
        ArrayList<BlendingData> list = Lists.newArrayList();
        ArrayList<BlendingData> list2 = Lists.newArrayList();
        ...
        for (int i = -HEIGHT_BLENDING_RANGE_CHUNKS; i <= HEIGHT_BLENDING_RANGE_CHUNKS; ++i) {
            for (int j = -HEIGHT_BLENDING_RANGE_CHUNKS; j <= HEIGHT_BLENDING_RANGE_CHUNKS; ++j) {
                ...
                if (blendingData == BlendingData.EMPTY) continue;
    ...
    public TerrainInfo blendOffsetAndFactor(int i, int j, TerrainInfo terrainInfo) {
        double d2;
        int k2 = SectionPos.blockToSectionCoord(i);
        int l2 = SectionPos.blockToSectionCoord(j);
        int m = QuartPos.fromBlock(i);
        int n = QuartPos.fromBlock(j);
        BlendingData blendingData = BlendingData.getOrCreateAndStoreToChunk(this.region, k2, l2);
        if (blendingData != BlendingData.EMPTY && (d2 = blendingData.getHeight(m, n)) != Double.POSITIVE_INFINITY) {
        ...
        for (BlendingData blendingData2 : this.heightData) {
            blendingData2.iterateHeights((k, l, d) -> {
                double e = Mth.length(m - k, n - l);
    ...
    public double blendDensity(int i, int j, int k, double d2) {
        double e;
        int l2 = SectionPos.blockToSectionCoord(i);
        int m2 = SectionPos.blockToSectionCoord(k);
        int n2 = QuartPos.fromBlock(i);
        int o = j / 8;
        int p = QuartPos.fromBlock(k);
        BlendingData blendingData = BlendingData.getOrCreateAndStoreToChunk(this.region, l2, m2);
        if (blendingData != BlendingData.EMPTY && (e = blendingData.getDensity(n2, o, p)) != Double.POSITIVE_INFINITY) {
            return e;
        }
        MutableDouble mutableDouble = new MutableDouble(0.0);
        MutableDouble mutableDouble2 = new MutableDouble(0.0);
        ...
        for (BlendingData blendingData2 : this.densityData) {
            blendingData2.iterateDensities(o - 2, o + 2, (l, m, n, d) -> {
                double e = Mth.length(n2 - l, o - m, p - n);
                if (e > 2.0) {
                    return;
                }
                if (e < mutableDouble3.doubleValue()) {
