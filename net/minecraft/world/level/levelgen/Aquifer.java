...
public class Aquifer {
    ...
    public Aquifer(int i, int j, NormalNoise normalNoise, NormalNoise normalNoise2, NoiseGeneratorSettings noiseGeneratorSettings, NoiseSampler noiseSampler, int k, int l) {
    ...
    protected void computeAt(int i, int j, int k) {
        ...
        u = this.getWaterLevel(r);
        v = this.getWaterLevel(s);
        w = this.getWaterLevel(t);
        ...
        if (this.lastWaterLevel >= j && Aquifer.isLavaLevel(j - this.noiseGeneratorSettings.noiseSettings().minY() - 1)) {
            this.lastBarrierDensity = 1.0;
        } else if (d > -1.0) {
            double g = 1.0 + (this.barrierNoise.getValue(i, j, k) + 0.05) / 4.0;
    ...
    private int getWaterLevel(long l) {
        ...
        this.aquiferCache[p] = r = this.computeAquifer(i, j, k);
        return r;
    }
