...
extends SurfaceBuilder<SurfaceBuilderBaseConfiguration> {
    ...
    public void apply(Random random, ChunkAccess chunkAccess, Biome biome, int i, int j, int k, double d, BlockState blockState, BlockState blockState2, int l, long m, SurfaceBuilderBaseConfiguration surfaceBuilderBaseConfiguration) {
        ...
        double f = this.soulSoilNoiseFloor.getValue(i, l, j);
        double g = this.soulSandNoiseFloor.getValue(i, l, j);
        double h = this.soulSoilNoiseCeiling.getValue(i, l, j);
        double s = this.soulSandNoiseCeiling.getValue(i, l, j);
        ...
        for (int t = 127; t >= 0; --t) {
            ...
            if ((blockState5.isAir() || blockState5 == blockState2) && blockState6.getBlock() == blockState.getBlock()) {
                for (u = 0; u < r && chunkAccess.getBlockState(mutableBlockPos).getBlock() == blockState.getBlock(); ++u) {
                    if (bl && t >= n - 4 && t <= n + 1) {
    ...
    public void initNoise(long l) {
        if (this.seed != l || this.soulSandNoiseFloor == null || this.soulSoilNoiseFloor == null || this.soulSandNoiseCeiling == null || this.soulSoilNoiseCeiling == null || this.gravelNoise == null) {
            this.soulSandNoiseFloor = new PerlinNoise(new WorldgenRandom(l), ImmutableList.of(Integer.valueOf(-4)));
            this.soulSoilNoiseFloor = new PerlinNoise(new WorldgenRandom(l + 1L), ImmutableList.of(Integer.valueOf(-4)));
            this.soulSandNoiseCeiling = new PerlinNoise(new WorldgenRandom(l + 2L), ImmutableList.of(Integer.valueOf(-4)));
            this.soulSoilNoiseCeiling = new PerlinNoise(new WorldgenRandom(l + 3L), ImmutableList.of(Integer.valueOf(-4)));
            this.gravelNoise = new PerlinNoise(new WorldgenRandom(l + 4L), ImmutableList.of(Integer.valueOf(0)));
        }
        this.seed = l;
    }
}
