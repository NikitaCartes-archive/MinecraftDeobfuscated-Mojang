...
extends CaveWorldCarver {
    public UnderwaterCaveWorldCarver(Function<Dynamic<?>, ? extends ProbabilityFeatureConfiguration> function) {
        super(function, 256);
    ...
    @Override
    protected boolean carveBlock(ChunkAccess chunkAccess, BitSet bitSet, Random random, BlockPos.MutableBlockPos mutableBlockPos, BlockPos.MutableBlockPos mutableBlockPos2, BlockPos.MutableBlockPos mutableBlockPos3, int i, int j, int k, int l, int m, int n, int o, int p, AtomicBoolean atomicBoolean) {
        return UnderwaterCaveWorldCarver.carveBlock(this, chunkAccess, bitSet, random, mutableBlockPos, i, j, k, l, m, n, o, p);
    }

    protected static boolean carveBlock(WorldCarver<?> worldCarver, ChunkAccess chunkAccess, BitSet bitSet, Random random, BlockPos.MutableBlockPos mutableBlockPos, int i, int j, int k, int l, int m, int n, int o, int p) {
        if (o >= i) {
            return false;
        }
        int q = n | p << 4 | o << 8;
        ...
        for (Direction direction : Direction.Plane.HORIZONTAL) {
            ...
            if (r >> 4 == j && s >> 4 == k && !chunkAccess.getBlockState(mutableBlockPos.set(r, o, s)).isAir()) continue;
