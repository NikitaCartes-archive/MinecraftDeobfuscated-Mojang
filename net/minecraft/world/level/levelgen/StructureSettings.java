...
public class StructureSettings {
    public static final Codec<StructureSettings> CODEC = RecordCodecBuilder.create(instance -> instance.group(StrongholdConfiguration.CODEC.optionalFieldOf("stronghold").forGetter(structureSettings -> Optional.ofNullable(structureSettings.stronghold)), Codec.simpleMap(Registry.STRUCTURE_FEATURE.byNameCodec(), StructureFeatureConfiguration.CODEC, Registry.STRUCTURE_FEATURE).fieldOf("structures").forGetter(structureSettings -> structureSettings.structureConfig)).apply((Applicative<StructureSettings, ?>)instance, StructureSettings::new));
    ...
    private StructureSettings(Map<StructureFeature<?>, StructureFeatureConfiguration> map, @Nullable StrongholdConfiguration strongholdConfiguration) {
        ...
        StructureFeatures.registerStructures((configuredStructureFeature, resourceKey) -> hashMap.computeIfAbsent(configuredStructureFeature.feature, structureFeature -> ImmutableMultimap.builder()).put(configuredStructureFeature, resourceKey));
    ...
    public StructureSettings(boolean bl) {
        this(Maps.newHashMap(DEFAULTS), bl ? DEFAULT_STRONGHOLD : null);
    }
    ...
    static {
        for (StructureFeature structureFeature : Registry.STRUCTURE_FEATURE) {
            if (DEFAULTS.containsKey(structureFeature)) continue;
