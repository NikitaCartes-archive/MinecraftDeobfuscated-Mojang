...
public class OreVeinifier {
    ...
    public OreVeinifier(long l, BlockState blockState, int i, int j, int k) {
        ...
        this.cellWidth = i;
        this.cellHeight = j;
        this.veinMaxY = Stream.of(VeinType.values()).mapToInt(veinType -> veinType.maxY).max().orElse(k);
        this.veinMinY = Stream.of(VeinType.values()).mapToInt(veinType -> veinType.minY).min().orElse(k);
    }
    ...
    private VeinType getVeinType(double d, int i) {
        ...
        int l = Math.min(j, k);
        double e = Mth.clampedMap(l, 0.0, 20.0, -0.2, 0.0);
        if (Math.abs(d) + e < 0.5) {
            return null;
        }
        return veinType;
