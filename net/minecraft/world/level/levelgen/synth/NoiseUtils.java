...
public class NoiseUtils {
    ...
    public static void parityNoiseOctaveConfigString(StringBuilder stringBuilder, double d, double e, double f, byte[] bs) {
        stringBuilder.append(String.format(Locale.ROOT, "xo=%.3f, yo=%.3f, zo=%.3f, p0=%d, p255=%d", Float.valueOf((float)d), Float.valueOf((float)e), Float.valueOf((float)f), bs[0], bs[255]));
    }

    public static void parityNoiseOctaveConfigString(StringBuilder stringBuilder, double d, double e, double f, int[] is) {
        stringBuilder.append(String.format(Locale.ROOT, "xo=%.3f, yo=%.3f, zo=%.3f, p0=%d, p255=%d", Float.valueOf((float)d), Float.valueOf((float)e), Float.valueOf((float)f), is[0], is[255]));
    }
}
