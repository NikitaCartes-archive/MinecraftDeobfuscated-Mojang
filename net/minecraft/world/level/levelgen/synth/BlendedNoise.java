...
implements DensityFunction.SimpleFunction {
    ...
    private static final MapCodec<BlendedNoise> DATA_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(((MapCodec)SCALE_RANGE.fieldOf("xz_scale")).forGetter(blendedNoise -> blendedNoise.xzScale), ((MapCodec)SCALE_RANGE.fieldOf("y_scale")).forGetter(blendedNoise -> blendedNoise.yScale), ((MapCodec)SCALE_RANGE.fieldOf("xz_factor")).forGetter(blendedNoise -> blendedNoise.xzFactor), ((MapCodec)SCALE_RANGE.fieldOf("y_factor")).forGetter(blendedNoise -> blendedNoise.yFactor), ((MapCodec)Codec.doubleRange(1.0, 8.0).fieldOf("smear_scale_multiplier")).forGetter(blendedNoise -> blendedNoise.smearScaleMultiplier)).apply((Applicative<BlendedNoise, ?>)instance, BlendedNoise::createUnseeded));
    ...
    public BlendedNoise(RandomSource randomSource, double d, double e, double f, double g, double h) {
        this(PerlinNoise.createLegacyForBlendedNoise(randomSource, IntStream.rangeClosed(-15, 0)), PerlinNoise.createLegacyForBlendedNoise(randomSource, IntStream.rangeClosed(-15, 0)), PerlinNoise.createLegacyForBlendedNoise(randomSource, IntStream.rangeClosed(-7, 0)), d, e, f, g, h);
    }
    ...
    public double compute(DensityFunction.FunctionContext functionContext) {
        ...
        for (int r = 0; r < 16; ++r) {
            ...
            double v = j * o;
            if (!bl2 && (improvedNoise2 = this.minLimitNoise.getOctaveNoise(r)) != null) {
                l += improvedNoise2.noise(s, t, u, v, e * o) / o;
            }
            if (!bl3 && (improvedNoise2 = this.maxLimitNoise.getOctaveNoise(r)) != null) {
