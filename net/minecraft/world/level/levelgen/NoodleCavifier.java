...
public class NoodleCavifier {
    ...
    public NoodleCavifier(long l) {
        ...
        this.toggleNoiseSource = NormalNoise.create((RandomSource)new SimpleRandomSource(random.nextLong()), -8, 1.0);
        this.thicknessNoiseSource = NormalNoise.create((RandomSource)new SimpleRandomSource(random.nextLong()), -8, 1.0);
        this.noodleANoiseSource = NormalNoise.create((RandomSource)new SimpleRandomSource(random.nextLong()), -7, 1.0);
        this.noodleBNoiseSource = NormalNoise.create((RandomSource)new SimpleRandomSource(random.nextLong()), -7, 1.0);
    }
    ...
    public void fillNoiseColumn(double[] ds, int i, int j, int k, int l, NormalNoise normalNoise, double d, double e) {
        ...
        for (int o = 0; o < l; ++o) {
            ...
            int r = p * 8;
            int s = j * 4;
            double f = r < 108 ? NoiseUtils.sampleNoiseAndMapToRange(normalNoise, (double)q * d, (double)r * e, (double)s * d, -1.0, 1.0) : 1.0;
            ds[o] = f;
        }
    }
    ...
    public double noodleCavify(double d, int i, int j, int k, double e, double f, double g, double h, int l) {
        ...
        double m = 0.05;
        double n = 0.07;
        double o = Mth.clampedMap(f, -1.0, 1.0, 0.05, 0.07);
