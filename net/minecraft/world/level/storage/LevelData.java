...
public class LevelData {
    ...
    private final TimerQueue<MinecraftServer> scheduledEvents = new TimerQueue<MinecraftServer>(TimerCallbacks.SERVER_CALLBACKS);
    ...
    public LevelData(CompoundTag compoundTag, DataFixer dataFixer, int i, @Nullable CompoundTag compoundTag2) {
        ...
        if (compoundTag.contains("generatorName", 8)) {
            ...
            Object levelType = LevelType.getLevelType(string);
            if (levelType == null) {
                levelType = LevelType.NORMAL;
            } else if (levelType == LevelType.CUSTOMIZED) {
                this.legacyCustomOptions = compoundTag.getString("generatorOptions");
            } else if (((LevelType)levelType).hasReplacement()) {
                int k = 0;
                if (compoundTag.contains("generatorVersion", 99)) {
                    k = compoundTag.getInt("generatorVersion");
                }
                levelType = ((LevelType)levelType).getReplacementForVersion(k);
            }
            CompoundTag compoundTag4 = compoundTag.getCompound("generatorOptions");
            Dynamic<CompoundTag> dynamic = new Dynamic<CompoundTag>(NbtOps.INSTANCE, compoundTag4);
            Dynamic<CompoundTag> dynamic2 = LevelData.datafixGeneratorOptions((LevelType)levelType, dynamic, i, dataFixer);
            this.generatorProvider = ((LevelType)levelType).createProvider(dynamic2);
        }
        ...
        this.generateMapFeatures = compoundTag.contains("MapFeatures", 99) ? compoundTag.getBoolean("MapFeatures") : true;
        ...
        this.dayTime = compoundTag.contains("DayTime", 99) ? compoundTag.getLong("DayTime") : this.gameTime;
        ...
        this.allowCommands = compoundTag.contains("allowCommands", 99) ? compoundTag.getBoolean("allowCommands") : this.gameType == GameType.CREATIVE;
    ...
    private void setTagData(CompoundTag compoundTag, CompoundTag compoundTag2) {
        ...
        compoundTag.putDouble("BorderWarningBlocks", this.borderWarningBlocks);
        compoundTag.putDouble("BorderWarningTime", this.borderWarningTime);
        ...
        for (Map.Entry<DimensionType, CompoundTag> entry : this.dimensionData.entrySet()) {
            compoundTag5.put(String.valueOf(entry.getKey().getId()), entry.getValue());
        }
    ...
    public void fillCrashReportCategory(CrashReportCategory crashReportCategory) {
        crashReportCategory.setDetail("Level name", () -> this.levelName);
        crashReportCategory.setDetail("Level seed", () -> String.valueOf(this.seed));
        crashReportCategory.setDetail("Level generator", () -> String.format("ID %02d - %s, ver %d. Features enabled: %b", this.generatorProvider.getType().getId(), this.generatorProvider.getType().getName(), this.generatorProvider.getType().getVersion(), this.generateMapFeatures));
        crashReportCategory.setDetail("Level generator options", () -> this.generatorProvider.getSettings().toString());
        crashReportCategory.setDetail("Level spawn location", () -> CrashReportCategory.formatLocation(this.xSpawn, this.ySpawn, this.zSpawn));
        crashReportCategory.setDetail("Level time", () -> String.format("%d game time, %d day time", this.gameTime, this.dayTime));
        crashReportCategory.setDetail("Known server brands", () -> String.join((CharSequence)", ", this.knownServerBrands));
        ...
        crashReportCategory.setDetail("Level weather", () -> String.format("Rain time: %d (now: %b), thunder time: %d (now: %b)", this.rainTime, this.raining, this.thunderTime, this.thundering));
        crashReportCategory.setDetail("Level game mode", () -> String.format("Game mode: %s (ID %d). Hardcore: %b. Cheats: %b", this.gameType.getName(), this.gameType.getId(), this.hardcore, this.allowCommands));
    }

    public CompoundTag getDimensionData(DimensionType dimensionType) {
        CompoundTag compoundTag = this.dimensionData.get(dimensionType);
        if (compoundTag == null) {
