    ...
    private <IS> TypeRewriteRule cap(Type<IS> type) {
        ...
        return this.fixTypeEverywhereTyped("EntityEquipmentToArmorAndHandFix", this.getInputSchema().getType(References.ENTITY), this.getOutputSchema().getType(References.ENTITY), (Typed<?> typed) -> {
            ...
            if (optional.isPresent()) {
                ...
                Object object = type.read(dynamic.emptyMap()).getSecond().orElseThrow(() -> new IllegalStateException("Could not parse newly created empty itemstack."));
                if (!list.isEmpty()) {
            ...
            Optional<Stream<Dynamic<?>>> optional2 = dynamic.get("DropChances").asStreamOpt();
            if (optional2.isPresent()) {
                ...
                if (!dynamic.get("HandDropChances").get().isPresent()) {
                    dynamic3 = dynamic.emptyMap().merge(dynamic.createFloat(f)).merge(dynamic.createFloat(0.0f));
                    dynamic = dynamic.set("HandDropChances", dynamic3);
                }
                if (!dynamic.get("ArmorDropChances").get().isPresent()) {
                    dynamic3 = dynamic.emptyMap().merge(dynamic.createFloat(((Dynamic)iterator.next()).asFloat(0.0f))).merge(dynamic.createFloat(((Dynamic)iterator.next()).asFloat(0.0f))).merge(dynamic.createFloat(((Dynamic)iterator.next()).asFloat(0.0f))).merge(dynamic.createFloat(((Dynamic)iterator.next()).asFloat(0.0f)));
