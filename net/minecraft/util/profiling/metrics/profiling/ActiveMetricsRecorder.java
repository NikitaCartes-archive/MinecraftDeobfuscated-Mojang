...
implements MetricsRecorder {
    ...
    private final Map<MetricSampler, List<RecordedDeviation>> deviationsBySampler = new Object2ObjectOpenHashMap<MetricSampler, List<RecordedDeviation>>();
    ...
    public static ActiveMetricsRecorder createStarted(MetricsSamplerProvider metricsSamplerProvider, LongSupplier longSupplier, Executor executor, MetricsPersister metricsPersister, Consumer<ProfileResults> consumer, Consumer<Path> consumer2) {
    ...
    public void endTick() {
        ...
        for (MetricSampler metricSampler2 : this.thisTickSamplers) {
            metricSampler2.onEndTick(this.currentTick);
            if (!metricSampler2.triggersThreshold()) continue;
            ...
            this.deviationsBySampler.computeIfAbsent(metricSampler2, metricSampler -> Lists.newArrayList()).add(recordedDeviation);
        }
    ...
    private void scheduleSaveResults(ProfileResults profileResults) {
        HashSet<MetricSampler> hashSet = new HashSet<MetricSampler>(this.thisTickSamplers);
