...
public class ProfilerSamplerAdapter {
    private final Set<String> previouslyFoundSamplerNames = new ObjectOpenHashSet<String>();

    public Set<MetricSampler> newSamplersFoundInProfiler(Supplier<ProfileCollector> supplier) {
        Set<MetricSampler> set = supplier.get().getChartedPaths().stream().filter(pair -> !this.previouslyFoundSamplerNames.contains(pair.getLeft())).map(pair -> ProfilerSamplerAdapter.samplerForProfilingPath(supplier, (String)pair.getLeft(), (MetricCategory)((Object)((Object)pair.getRight())))).collect(Collectors.toSet());
